...
function renderLineETAs(){
  ...
  LINES.forEach(function(l){
    ...
    var eta = calcLineETA(l.id);
    if(!eta) return;

    var hrs = parseFloat(eta.hours);
    var shiftLeft = Math.max(0, shiftHours() - elapsed());
    var col = hrs <= shiftLeft ? 'var(--green)' : 'var(--red)';
    var status = hrs <= shiftLeft ? 'יסיים בזמן' : 'חריגה צפויה';

    html += '<div style="background:var(--card);border:1px solid var(--border);border-radius:10px;padding:12px 14px;margin-bottom:8px">';
    html += '<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:6px">';
    html += '<span style="font-size:14px;font-weight:700">' + l.name + '</span>';
    html += '<span style="font-size:12px;padding:2px 10px;background:' + (hrs<=shiftLeft?'var(--green-bg)':'var(--red-bg)') + ';color:' + col + ';border-radius:10px;font-weight:700">' + status + '</span>';
    html += '</div>';
    html += '<div style="display:grid;grid-template-columns:repeat(4,1fr);gap:8px">';
    html += '<div style="text-align:center"><div style="font-size:20px;font-weight:800;font-family:var(--mono);color:' + col + '">' + eta.hours + '</div><div style="font-size:10px;color:var(--dim)">שעות (עם כא"ד)</div></div>';
    html += '<div style="text-align:center"><div style="font-size:20px;font-weight:800;font-family:var(--mono);color:var(--dim)">' + shiftLeft.toFixed(1) + '</div><div style="font-size:10px;color:var(--dim)">שעות שנותרו</div></div>';
    html += '<div style="text-align:center"><div style="font-size:20px;font-weight:800;font-family:var(--mono);color:var(--blue)">' + eta.workers + '</div><div style="font-size:10px;color:var(--dim)">עובדים</div></div>';
    html += '<div style="text-align:center"><div style="font-size:20px;font-weight:800;font-family:var(--mono);color:var(--yellow)">' + eta.openRows + '</div><div style="font-size:10px;color:var(--dim)">שורות פתוחות</div></div>';
    html += '</div>';
    if(eta.unknownQty > 0){
      html += '<div style="font-size:11px;color:var(--yellow);margin-top:6px">' + eta.unknownQty + ' יח ללא נתוני זמן בקטלוג</div>';
    }
    html += '</div>';
  });
...
// ============================================================
// 2. Catalog management - add/remove/edit SKUs
// ============================================================
function saveCatalogEdits(){
  try{
    var edits = JSON.parse(store.getItem('ops_catalog_edits') || '[]');
    return edits;
  } catch(e){ return []; }
}
...
function renderCatalogManager(){
  ...
  var LINE_NAMES = {pick:'ליקוט',drive:'יח\' הנעה',kit:'ערכות',cable:'כבלים',spk:'ספקי כוח'};

  var html = '<div style="margin-bottom:12px">';
  html += '<div style="font-size:13px;font-weight:700;margin-bottom:8px">הוסף / עדכן מק"ט</div>';
  html += '<div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-bottom:8px">';
  html += '<input type="text" id="cat-sku" placeholder="מק"ט" autocapitalize="characters" style="background:var(--surface);border:1px solid var(--border);border-radius:7px;padding:7px 10px;color:var(--text);font-size:13px;font-family:var(--mono)">';
  html += '<select id="cat-line" style="background:var(--surface);border:1px solid var(--border);border-radius:7px;padding:7px 10px;color:var(--text);font-size:13px;font-family:Heebo,sans-serif">';
  html += '<option value="">בחר נתיב</option>';
  Object.entries(LINE_NAMES).forEach(function(e){ html += '<option value="' + e[0] + '">' + e[1] + '</option>'; });
  html += '</select>';
  html += '<input type="text" id="cat-desc" placeholder="תיאור מוצר" style="background:var(--surface);border:1px solid var(--border);border-radius:7px;padding:7px 10px;color:var(--text);font-size:13px">';
  html += '<input type="number" id="cat-dur" placeholder="זמן ייצור (דקות)" step="0.01" style="background:var(--surface);border:1px solid var(--border);border-radius:7px;padding:7px 10px;color:var(--text);font-size:13px">';
  html += '</div>';
  ...
  // Custom edits list
  if(edits.length){
    html += '<div style="font-size:13px;font-weight:700;margin-bottom:6px">מק"טים מותאמים אישית (' + edits.length + ')</div>';
    edits.forEach(function(item, i){
      ...
      html += '<div style="font-size:11px;color:var(--dim)">' + (item.desc||'') + ' | ' + (LINE_NAMES[item.lineId]||item.lineId||'') + ' | ' + (item.duration||0) + ' דק</div></div>';
