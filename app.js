...
var targets={},reports={},reportsUpdatedAt={},redRows={},redClosed={},stoppages=[],alertsLog=[],history={};
...
function clearDay(){
  ...
  targets={};reports={};reportsUpdatedAt={};redRows={};redClosed={};stoppages=[];palletRequests=[];orderRows={};
...
function syncToServer(){
  ...
  sbFetch('ops_data?select=key,value&key=in.(stoppages,pallets,reports,reports_ts,order_rows)').then(function(rows){
...
function openStop(){
  ...
  stoppages.unshift({id:Date.now().toString(),lineName:line,reason:reason,startTime:nowTime(),startTs:Date.now(),endTime:null,durationMin:null,open:true,updatedAt:Date.now()});
...
function saveEditStop(){
  ...
  if(isNew){stoppages.unshift({id:Date.now().toString(),lineName:line,reason:reason,startTime:start,startTs:sd.getTime(),endTime:end||null,durationMin:dur,open:open,updatedAt:Date.now()});}
  else{var s2=stoppages.filter(function(x){return x.id===id;})[0];if(s2)Object.assign(s2,{lineName:line,reason:reason,startTime:start,startTs:sd.getTime(),endTime:end||null,durationMin:dur,open:open,updatedAt:Date.now()});}
