...
function syncFromServer(){
  ...
  sbFetch('ops_data?select=key,value').then(function(rows){
    ...
    saveState();
    // Don't re-render dash if user is searching in open line
    if(!(window._dashOpenLine&&window._dashSearch)){renderDash();}
    else{renderDashRows();}
...
function renderDash(){
  ...
  if(searchInp){
    ...
    if(savedVal){
      searchInp.focus();
      try{searchInp.setSelectionRange(savedVal.length,savedVal.length);}catch(e){}
    }
