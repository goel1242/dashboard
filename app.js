  ...
  cardsHtml+=calcs.map(function(c){
    ...
    if(matArrived.length){
      ...
      strips+='<div style="background:#F0FDF4;border:1px solid rgba(5,150,105,.3);border-radius:5px;padding:2px 4px;margin-bottom:2px;display:flex;justify-content:space-between;gap:3px;align-items:center">'+
        '<span style="font-size:8.5px;color:#059669;font-weight:700;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">'+ma.sku+'</span>'+
        '<span style="font-size:8.5px;color:#059669;white-space:nowrap">\u2713 \u05d4\u05d2\u05d9\u05e2'+(matArrived.length>1?' +'+(matArrived.length-1):'')+'</span>'+
      '</div>';
    }
...
function planKey(lineId,sku){return lineId+'|'+sku;}
function renderSetupPlan(){
  var el=document.getElementById('setup-subcontent');
  if(!el) return;
  var PLAN_LINES=[{id:'drive',name:'\u05d9\u05d7\u05d9\u05d3\u05d5\u05ea \u05d4\u05e0\u05e2\u05d4'},{id:'spk',name:'\u05e1\u05e4\u05e7\u05d9 \u05db\u05d5\u05d7'},{id:'cable',name:'\u05db\u05d1\u05dc\u05d9\u05dd'}];
  var html='<div style="font-size:10.5px;color:var(--faint);text-align:center;margin-bottom:12px">\u05de\u05e7\u05d5\u05d1\u05e5 \u05d0\u05d5\u05d8\u05d5\u05de\u05d8\u05d9\u05ea \u05de\u05ea\u05d5\u05db\u05e0\u05d9\u05ea \u05d4\u05e2\u05d1\u05d5\u05d3\u05d4 \u05e9\u05dc \u05d4\u05d9\u05d5\u05dd \u00b7 \u05dc\u05e4\u05d9 \u05de\u05e7"\u05d8 \u05d1\u05df</div>';
  var anyData=false;
  PLAN_LINES.forEach(function(pl){
    var rows=orderRows[pl.id]||[];
    var groups={};
    rows.forEach(function(r){
      var childSku=(r.skuB||'').trim();
      if(!childSku) return;
      groups[childSku]=(groups[childSku]||0)+(r.qty||0);
    });
    var keys=Object.keys(groups);
    if(!keys.length) return;
    anyData=true;
    keys.sort(function(a,b){return groups[b]-groups[a];});
    var openKeys=keys.filter(function(k){var st=planItemStatus[planKey(pl.id,k)];return !st||!st.status;});
    var handledKeys=keys.filter(function(k){var st=planItemStatus[planKey(pl.id,k)];return st&&st.status;});
    ...
    openKeys.forEach(function(k){
      var key=planKey(pl.id,k),qty=groups[k];
      html+='<div style="background:var(--card);border:1px solid var(--border);border-radius:10px;padding:10px 12px;margin-bottom:8px">'+
        '<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px">'+
          '<span style="font-size:12.5px;font-weight:700;font-family:var(--mono)">'+escapeHtml(k)+'</span>'+
          '<span style="font-size:13px;font-weight:800;color:var(--text);font-family:var(--mono)">\u05db\u05de\u05d5\u05ea '+qty+'</span>'+
        '</div>'+
        '<div style="display:flex;gap:5px">'+
          '<div onclick="planMarkFull(\''+key+'\')" style="flex:1;text-align:center;padding:7px 3px;background:var(--green-bg);border:1px solid rgba(5,150,105,.3);border-radius:6px;color:var(--green);font-size:11px;font-weight:700;cursor:pointer">\u05d4\u05d2\u05d9\u05e2 \u05d4\u05db\u05dc</div>'+
          '<div onclick="planTogglePartial(\''+key+'\')" style="flex:1;text-align:center;padding:7px 3px;background:var(--yellow-bg);border:1px solid rgba(217,119,6,.3);border-radius:6px;color:var(--yellow);font-size:11px;font-weight:700;cursor:pointer">\u05d7\u05dc\u05e7\u05d9</div>'+
          '<div onclick="planMarkNoStock(\''+key+'\')" style="flex:1;text-align:center;padding:7px 3px;background:var(--red-bg);border:1px solid rgba(220,38,38,.3);border-radius:6px;color:var(--red);font-size:11px;font-weight:700;cursor:pointer">\u05d0\u05d9\u05df \u05de\u05dc\u05d0\u05d9</div>'+
        '</div>'+
        '<div id="plan-partial-'+key.replace(/\|/g,'_')+'" style="display:none;margin-top:8px;background:var(--yellow-bg);border-radius:8px;padding:9px">'+
          '<div style="font-size:10px;color:var(--yellow);font-weight:700;margin-bottom:6px">\u05db\u05de\u05d4 \u05d4\u05d2\u05d9\u05e2 \u05d1\u05e4\u05d5\u05e2\u05dc?</div>'+
          '<div style="display:flex;gap:8px;align-items:center;margin-bottom:8px">'+
            '<input type="number" id="plan-partial-qty-'+key.replace(/\|/g,'_')+'" placeholder="0" inputmode="numeric" style="flex:1;background:var(--card);border:2px solid var(--yellow);border-radius:7px;padding:7px;font-size:15px;font-family:var(--mono);text-align:center;color:var(--text)">'+
            '<span style="font-size:11px;color:var(--dim)">\u05de\u05ea\u05d5\u05da '+qty+'</span>'+
          '</div>'+
          '<div style="display:flex;gap:6px">'+
            '<button onclick="planConfirmPartial(\''+key+'\')" style="flex:1;padding:7px;background:var(--yellow);border:none;border-radius:6px;color:#fff;font-size:11px;font-weight:700;cursor:pointer">\u05d0\u05e9\u05e8 \u05db\u05de\u05d5\u05ea \u05d7\u05dc\u05e7\u05d9\u05ea</button>'+
            '<button onclick="planTogglePartial(\''+key+'\')" style="padding:7px 10px;background:var(--card);border:1px solid var(--border);border-radius:6px;color:var(--dim);font-size:11px;font-weight:700;cursor:pointer">\u05d1\u05d9\u05d8\u05d5\u05dc</button>'+
          '</div>'+
        '</div>'+
      '</div>';
      var stChk=planItemStatus[key];
      if(!stChk||!stChk.seenAt) planMarkSeen(key);
    });
    handledKeys.forEach(function(k){
      var key=planKey(pl.id,k),qty=groups[k],st=planItemStatus[key];
      var isNoStock=st.status==='nostock',isPartial=st.status==='partial';
      var col=isNoStock?'var(--red)':isPartial?'var(--yellow)':'var(--green)';
      var bg=isNoStock?'var(--red-bg)':isPartial?'var(--yellow-bg)':'var(--green-bg)';
      var bdr=isNoStock?'rgba(220,38,38,.25)':isPartial?'rgba(217,119,6,.25)':'rgba(5,150,105,.25)';
      var lbl=isNoStock?'\u05d0\u05d9\u05df \u05de\u05dc\u05d0\u05d9':isPartial?'\u05d4\u05d2\u05d9\u05e2 '+(st.arrivedQty||0)+'/'+qty:'\u05d4\u05d2\u05d9\u05e2 \u05d4\u05db\u05dc';
      html+='<div style="background:'+bg+';border:1px solid '+bdr+';border-radius:10px;padding:9px 12px;margin-bottom:6px;display:flex;justify-content:space-between;align-items:center;opacity:.9">'+
        '<div><div style="font-size:12px;font-weight:700;color:'+col+';font-family:var(--mono)">'+escapeHtml(k)+'</div><div style="font-size:10.5px;color:'+col+'">'+lbl+
        (st.seenAt?' \u00b7 \u2713 \u05e0\u05e6\u05e4\u05d4 '+(st.seenTime||''):'')+'</div></div>'+
        '<button onclick="planEditStatus(\''+key+'\')" style="font-size:9px;padding:3px 8px;background:var(--card);border:1px solid var(--border);border-radius:5px;color:var(--faint);cursor:pointer">\u05e2\u05e8\u05d9\u05db\u05d4</button>'+
      '</div>';
    });
    html+='</div>';
  });
...
function planTogglePartial(key){
  var box=document.getElementById('plan-partial-'+key.replace(/\|/g,'_'));
  if(!box) return;
  box.style.display=box.style.display==='none'||!box.style.display?'block':'none';
}
function planMarkFull(key){
  if(!planItemStatus[key]) planItemStatus[key]={};
  planItemStatus[key].status='arrived';
  planItemStatus[key].arrivalTs=Date.now();planItemStatus[key].arrivalTime=nowTime();
  savePlanStatus();renderSetupScreen();
  showT('\u05e1\u05d5\u05de\u05df \u05d4\u05d2\u05d9\u05e2 \u05d4\u05db\u05dc!');
}
function planConfirmPartial(key){
  var elId=key.replace(/\|/g,'_');
  var inp=document.getElementById('plan-partial-qty-'+elId);
  if(!inp) return;
  var qty=parseInt(inp.value)||0;
  if(qty<=0){showT('\u05d4\u05d6\u05df \u05db\u05de\u05d5\u05ea \u05e9\u05d4\u05d2\u05d9\u05e2\u05d4','y');return;}
  if(!planItemStatus[key]) planItemStatus[key]={};
  planItemStatus[key].status='partial';
  planItemStatus[key].arrivedQty=qty;
  planItemStatus[key].arrivalTs=Date.now();planItemStatus[key].arrivalTime=nowTime();
  savePlanStatus();renderSetupScreen();
  showT('\u05e1\u05d5\u05de\u05df \u05db\u05d7\u05dc\u05e7\u05d9!');
}
function planMarkNoStock(key){
  if(!planItemStatus[key]) planItemStatus[key]={};
  planItemStatus[key].status='nostock';
  planItemStatus[key].arrivalTs=Date.now();planItemStatus[key].arrivalTime=nowTime();
  savePlanStatus();renderSetupScreen();
  showT('\u05e1\u05d5\u05de\u05df \u05d0\u05d9\u05df \u05de\u05dc\u05d0\u05d9');
}
function planEditStatus(key){
  if(planItemStatus[key]){
    planItemStatus[key]={seenAt:planItemStatus[key].seenAt,seenTime:planItemStatus[key].seenTime};
  }
  savePlanStatus();renderSetupScreen();
  showT('\u05e0\u05e4\u05ea\u05d7 \u05dc\u05e2\u05e8\u05d9\u05db\u05d4 \u05de\u05d7\u05d3\u05e9','y');
}
function planMarkSeen(key){
  if(!planItemStatus[key]) planItemStatus[key]={};
  if(planItemStatus[key].seenAt) return;
  planItemStatus[key].seenAt=Date.now();planItemStatus[key].seenTime=nowTime();
  savePlanStatus();
}
