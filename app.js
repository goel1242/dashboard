  ...
  try{
    ...
    try{
      ...
      Object.keys(_orAll).forEach(function(lid){
        var rows=_orAll[lid].filter(function(r){return (r.done||0)>0||r.note;});
...
function syncFromServer(){
  ...
  sbFetch('ops_data?select=key,value').then(function(rows){
    ...
    if(map.stoppages){try{
      var localSTS2=window._stoppagesTS||parseInt(store.getItem('ops_stops_ts')||'0');
      // Only overwrite if we haven't recorded any local stoppage actions
      if(localSTS2===0) stoppages=JSON.parse(map.stoppages);
      else {
        // Merge: keep local open stoppages, add remote closed ones
        var remoteST=JSON.parse(map.stoppages);
        var localOpen=stoppages.filter(function(s){return s.open;});
        var remoteClosed=remoteST.filter(function(s){return !s.open;});
        var localIds=stoppages.map(function(s){return s.id;});
        remoteClosed.forEach(function(s){if(localIds.indexOf(s.id)<0) stoppages.push(s);});
      }
    }catch(e){}}
...
function addPallet(){
  window._lastUserAction=Date.now();
  var id='P'+Date.now();
  pallets.unshift({id:id,orderId:'',customer:'',pkNum:'',shipType:'sea',maxHeight:220,special:false,specialNote:'',cartons:{dolbox:0,upgrade:0,box2x2:0},sourceLogs:[],closed:false,createdAt:nowTime()});
...
function genShiftSummary(){
  ...
  LINES.forEach(function(l){
    (orderRows[l.id]||[]).forEach(function(r){
      if(r.note&&!(r.done>=r.qty)) _reasonLines.push(l.name+' | '+r.order+': '+r.note);
    });
  });
...
function buildHistExportRows(dateKey,d){
  ...
  Object.keys(or).forEach(function(lid){
    ...
    (or[lid]||[]).forEach(function(r){
      out.push([dateKey,r.order||'',r.orderLine||'',r.sku||'',r.desc||'',r.customer||'',lineName,r.qty||0,r.note||'',r.completedAt||'']);
    });
  });
  return out;
}
function exportHistoryDay(dateKey){
  ...
  var aoa=[['תאריך','הזמנה','שורה','מק"ט','תאור','לקוח','נתיב','כמות','סיבת אי ביצוע','שעה']].concat(rows);
...
function exportHistoryRange(fromDate,toDate){
  ...
  var aoa=[['תאריך','הזמנה','שורה','מק"ט','תאור','לקוח','נתיב','כמות','סיבת אי ביצוע','שעה']];
...
setInterval(function(){
  sbFetch('ops_data').then(function(data){
    ...
    if(map.stoppages) try{
      var localSTS=window._stoppagesTS||parseInt(store.getItem('ops_stops_ts')||'0');
      var remoteSTS=0; // stoppages has no remote TS yet - only overwrite if local is old
      if(localSTS===0) stoppages=JSON.parse(map.stoppages);
    }catch(e){}
    if(map.pallets) try{pallets=JSON.parse(map.pallets);}catch(e){}
