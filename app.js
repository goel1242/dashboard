...
function syncFromServer(){
  ...
  sbFetch('ops_data?select=key,value').then(function(rows){
    ...
    if(!_recentSearch){renderDash();}
    updateStopBdg();updateAlertBdg();
    if(document.getElementById('page-pack').classList.contains('active')){var ae3=document.activeElement;var isTypingPack=ae3&&(ae3.tagName==='INPUT'||ae3.tagName==='TEXTAREA'||ae3.tagName==='SELECT');if(!isTypingPack){buildRequestForms();renderPallets();renderPackRequests();}}
...
function renderDash(){
  ...
  if(openLineId){
    ...
    if(openCalc&&openLine){
      ...
      inlineRows+='<div style="display:flex;gap:5px;margin-bottom:6px;flex-wrap:nowrap;overflow-x:auto;-webkit-overflow-scrolling:touch">';
      [['all','הכל'],['order','הזמנה'],['sku','מק\"ט'],['skuB','מק\"ט בן'],['expiry','פק\"ע']].forEach(function(f){
        var active=_sf===f[0];
        inlineRows+='<button data-field="'+f[0]+'" onclick="window._dashSearchField=this.dataset.field;document.getElementById(\'dash-row-search\').value=\'\';window._dashSearch=\'\';filterDashRows(\'\');this.parentElement.querySelectorAll(\'button\').forEach(function(b){b.style.background=b.dataset.field===window._dashSearchField?\'#2563EB\':\'var(--surface)\';b.style.color=b.dataset.field===window._dashSearchField?\'#fff\':\'var(--dim)\';b.style.borderColor=b.dataset.field===window._dashSearchField?\'#2563EB\':\'var(--border)\';});" style="font-size:10px;padding:5px 10px;border-radius:16px;border:1.5px solid '+(active?'#2563EB':'var(--border)')+';background:'+(active?'#2563EB':'var(--surface)')+';color:'+(active?'#fff':'var(--dim)')+';white-space:nowrap;flex-shrink:0;cursor:pointer;font-family:var(--font)">'+f[1]+'</button>';
      });
      inlineRows+='</div>';
