...
function sbFetch(path,opts){
  ...
  return fetch(SB_URL+'/rest/v1/'+path,{
    ...
    if(!r.ok && r.status!==204){
      return r.json().then(function(err){throw new Error(JSON.stringify(err));}).catch(function(){throw new Error('HTTP '+r.status);});
    }
...
setInterval(function(){
  sbFetch('ops_data').then(function(data){
    ...
    if(map.targets) try{targets=JSON.parse(map.targets);}catch(e){}
    ...
    // Update KPIs and badge only - no full render
    updateDashKPIs();
    updateStopBdg();
    updateAlertBdg();
  }).catch(function(){});
