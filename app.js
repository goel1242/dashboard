...
function filterPallets(termOverride){
  ...
  var orig=pallets;
  pallets=pallets.filter(function(p){return palletMatchesTerm(p,term);});
  renderPallets();
  pallets=orig;
}
function refreshPalletSelect(){
  ...
  if(!el) return;
  var sEl=document.getElementById('req-pallet-search');
  var term=sEl?sEl.value.trim().toLowerCase():'';
  var cur=el.value;
  var active=pallets.filter(function(p){return !p.closed;});
  if(term){
    active=active.filter(function(p){return palletMatchesTerm(p,term);});
  }
  ...
  else{opts+='<option value="" disabled>'+(term?'אין תוצאות':'אין משטחים פתוחים')+'</option>';}
  el.innerHTML=opts;
  if(active.some(function(p){return p.id===cur;})) el.value=cur;
  // Keep the visible pallet cards list below in sync with this search too
  filterPallets(term);
}
...
function buildRequestForms(){
  ...
  el.innerHTML='<div class="ic">'+
    '<div class="igrid" style="margin-bottom:8px">'+
      ...
      '<div class="fld"><label>משטח יעד</label>'+
        '<input type="text" id="req-pallet-search" inputmode="text" autocomplete="off" autocorrect="off" autocapitalize="off" spellcheck="false" placeholder="חפש הזמנה/לקוח/PK..." oninput="refreshPalletSelect()" style="background:var(--surface);border:1px solid var(--border);border-radius:8px;padding:6px 8px;color:var(--text);font-size:12px;width:100%;margin-bottom:4px">'+
        '<select id="req-pallet" onfocus="refreshPalletSelect()" style="'+is+'"><option value="">-- בחר --</option></select>'+
      '</div>'+
    '</div>'+
    '<div class="igrid" style="margin-bottom:8px">'+
