...
function syncToServer(){
  ...
  var keys=[
    ...
    {key:'order_rows',value:JSON.stringify({_ts:window._orderRowsTS||Date.now(),data:orderRows})}
  ].map(function(k){return{key:k.key,value:k.value,updated_at:ts};});
  // Send in two batches: small keys first, then order_rows
  var smallKeys=keys.slice(0,9);
  var bigKeys=keys.slice(9);
  sbFetch('ops_data',{method:'POST',prefer:'resolution=merge-duplicates',body:JSON.stringify(smallKeys)})
    .then(function(){
      ...
      // Add local timestamp to order_rows
      var orKey=bigKeys.find(function(k){return k.key==='order_rows';});
      if(orKey) orKey.ts=window._orderRowsTS||Date.now();
      return sbFetch('ops_data',{method:'POST',prefer:'resolution=merge-duplicates',body:JSON.stringify(bigKeys)});
    })
    .catch(function(e){
...
function syncFromServer(){
  ...
  sbFetch('ops_data?select=key,value').then(function(rows){
    ...
    if(map.order_rows){try{
      var _orParsed=JSON.parse(map.order_rows);
      var sr2=_orParsed.data||_orParsed; // support both formats
      var remoteTS2=_orParsed._ts||0;
      if(Object.keys(sr2).length>0){
        var localTS2=window._orderRowsTS||parseInt(store.getItem('ops_order_rows_ts')||'0');
        if(remoteTS2>localTS2||localTS2===0){
          orderRows=sr2;recalcRedRows();
          window._orderRowsTS=remoteTS2;
          store.setItem('ops_order_rows_ts',String(remoteTS2));
        }
      }
    }catch(e){}}
    if(map.history){try{
...
function renderDash(){
  ...
  cardsHtml+=calcs.map(function(c){
    ...
    return '<button data-lineid="'+c.id+'" style="background:'+cardBg+';border:'+cardBorder+';border-radius:12px;padding:10px 8px;cursor:pointer;text-align:center;-webkit-tap-highlight-color:transparent;width:100%;font-family:Heebo,sans-serif" onclick="dashToggleLine(this.dataset.lineid)">'+
...
setInterval(function(){
  sbFetch('ops_data').then(function(data){
    ...
    if(map.order_rows) try{
      ...
      var sr=_orBG.data||_orBG;
      var remoteTS=_orBG._ts||0;
      if(Object.keys(sr).length>0){
        var localTS=window._orderRowsTS||parseInt(store.getItem('ops_order_rows_ts')||'0');
        if(remoteTS>localTS||localTS===0){
          orderRows=sr;recalcRedRows();
          window._orderRowsTS=remoteTS;
          store.setItem('ops_order_rows_ts',String(remoteTS));
        }
      }
    }catch(e){}
    if(map.stoppages) try{
