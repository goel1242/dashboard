...
function renderDash(){
  ...
  cardsHtml+=calcs.map(function(c){
    ...
    var matIssues=(typeof materialRequests!=='undefined'?materialRequests:[]).filter(function(r){return r.lineId===c.id&&(r.status==='nostock'||r.status==='partial');});
    if(matIssues.length){
      var mi=matIssues[0];
      var miNoStock=mi.status==='nostock';
      var miCol=miNoStock?'#DC2626':'#D97706';
      var miBg=miNoStock?'#FFF0F0':'#FFFBEB';
      var miBdr=miNoStock?'rgba(220,38,38,.3)':'rgba(217,119,6,.3)';
      var miLbl=miNoStock?'אין מלאי':('הגיע '+(mi.arrivedQty||0)+'/'+mi.qty);
      strips+='<div style="background:'+miBg+';border:1px solid '+miBdr+';border-radius:5px;padding:2px 4px;margin-bottom:2px;display:flex;justify-content:space-between;gap:3px;align-items:center">'+
        '<span style="font-size:8.5px;color:'+miCol+';font-weight:700;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">'+mi.sku+'</span>'+
        '<span style="font-size:8.5px;color:'+miCol+';white-space:nowrap">'+miLbl+(matIssues.length>1?' +'+(matIssues.length-1):'')+'</span>'+
      '</div>';
      if(!mi.seenAt&&typeof markMaterialSeen==='function') markMaterialSeen(mi.id);
      else if(mi.seenAt) strips+='<div style="font-size:7.5px;color:'+miCol+';text-align:center;margin-top:-1px;margin-bottom:2px">\u2713 \u05e0\u05e6\u05e4\u05d4</div>';
    }
    var _todayStr=new Date().toDateString();
    var matArrived=(typeof materialRequests!=='undefined'?materialRequests:[]).filter(function(r){
      if(r.lineId!==c.id||r.status!=='arrived') return false;
      var rows=((typeof orderRows!=='undefined'?orderRows[c.id]:null)||[]);
      var rSku=(r.sku||'').trim().toLowerCase();
      var matchingRows=rows.filter(function(row){return (row.sku||'').trim().toLowerCase()===rSku;});
      if(matchingRows.length){
        // Tied to a real plan row - keep showing until that specific row is fully done
        return matchingRows.some(function(row){return (row.done||0)<(row.qty||0);});
      }
      // Not tied to any row in today's work plan (e.g. a sub-assembly / accompanying part
      // that's ordered ad-hoc, not listed in the plan) - still confirm it arrived, for the
      // rest of the day it was requested, since there's no plan row to track completion of
      var reqDay=new Date(r.createdTs||Date.now()).toDateString();
      return reqDay===_todayStr;
    });
    // Also reflect rows marked arrived from the Setup screen's "שורות אדומות" and "תוכנית יומי"
    // tabs (both write to the same redRowStatus, keyed by lineId|order|orderLine) - without
    // this, marking material arrived there never showed anything on the dashboard at all.
    var rowArrivedSkus=[];
    if(typeof redRowStatus!=='undefined'){
      var _rowsForLine=((typeof orderRows!=='undefined'?orderRows[c.id]:null)||[]);
      _rowsForLine.forEach(function(row){
        if((row.done||0)>=(row.qty||0)) return; // row itself already fully done - nothing to reflect
        var rk=c.id+'|'+(row.order||'')+'|'+(row.orderLine||'');
        var st=redRowStatus[rk];
        if(st&&st.status==='arrived') rowArrivedSkus.push(row.sku||row.skuB||'');
      });
    }
    var totalArrived=matArrived.length+rowArrivedSkus.length;
    if(totalArrived>0){
      var firstArrivedSku=matArrived.length?matArrived[0].sku:rowArrivedSkus[0];
      strips+='<div style="background:#F0FDF4;border:1px solid rgba(5,150,105,.3);border-radius:5px;padding:2px 4px;margin-bottom:2px;display:flex;justify-content:space-between;gap:3px;align-items:center">'+
        '<span style="font-size:8.5px;color:#059669;font-weight:700;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">'+firstArrivedSku+'</span>'+
        '<span style="font-size:8.5px;color:#059669;white-space:nowrap">\u2713 \u05d4\u05d2\u05d9\u05e2'+(totalArrived>1?' +'+(totalArrived-1):'')+'</span>'+
      '</div>';
    }
...
function renderSetupRequests(){
  ...
  var open=materialRequests.filter(function(r){return r.status==='pending'||r.status==='inProgress';});
