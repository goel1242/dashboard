...
function renderDash(){
  ...
  if(!openLineId){
    ...
    // Only rebuild if section is empty or line changed
    var _existingLine=_inlineSection.getAttribute('data-line');
    if(_existingLine===openLineId && _inlineSection.innerHTML.length>100){
      // Already showing correct line - skip rebuild
    } else {
      _inlineSection.setAttribute('data-line',openLineId);
      _inlineSection.innerHTML='<div style="padding:16px;text-align:center;color:var(--dim);font-size:13px">טוען...</div>';
      setTimeout(function(){
      ...
      _inlineSection.scrollIntoView({behavior:'smooth',block:'start'});
      },0);
    }
  }
...
function buildDashInlineRows(openLineId){
  ...
  html+='<input type="search" id="dash-row-search" autocomplete="off" autocorrect="off" autocapitalize="off" spellcheck="false" placeholder="חפש..." value="'+(window._dashSearch||'')+'" style="flex:1;background:var(--surface);border:1px solid var(--border);border-radius:8px;padding:7px 12px;color:var(--text);font-size:13px">';
  html+='<button onclick="var v=document.getElementById(\'dash-row-search\').value;window._dashSearch=v;filterDashRows(v);" style="padding:7px 14px;background:var(--blue);border:none;border-radius:8px;color:#fff;font-size:13px;font-weight:700;cursor:pointer;white-space:nowrap">חפש</button>';
  html+='</div>';
  {
    ...
    if(openCalc&&openLine){
      ...
      filtered2.forEach(function(r){
        ...
        if(isDone){
          ...
          html+='<div style="background:#fff;border:1px solid rgba(34,197,94,.3);border-radius:6px;padding:2px 8px;font-size:9px;color:#16A34A;font-weight:700">הושלם ב-'+(r.completedAt||'')+'</div>';
        ...
        if(!isDone){
          if(r.rowReason){
            html+='<div style="background:#F0FDF4;border:1px solid rgba(5,150,105,.25);border-radius:8px;padding:7px 10px;margin-top:8px;display:flex;justify-content:space-between;align-items:flex-start">';
            html+='<div><div style="font-size:10px;color:#64748B;margin-bottom:2px">סיבת אי ביצוע</div><div style="font-size:12px;font-weight:700;color:#059669">'+r.rowReason+'</div>'+(r.rowNote?'<div style="font-size:11px;color:#334155;margin-top:2px">'+r.rowNote+'</div>':'')+'</div>';
            html+='<button data-lid="'+openLineId+'" data-idx="'+realIdx+'" onclick="toggleDashRowReason(this.dataset.lid,parseInt(this.dataset.idx))" style="font-size:10px;padding:2px 8px;background:#fff;border:1px solid #E2E8F0;border-radius:5px;color:#64748B;cursor:pointer">ערוך</button>';
            html+='</div>';
          } else {
            html+='<button data-lid="'+openLineId+'" data-idx="'+realIdx+'" onclick="toggleDashRowReason(this.dataset.lid,parseInt(this.dataset.idx))" style="width:100%;font-size:11px;padding:5px;background:#F8FAFC;border:1px solid #E2E8F0;border-radius:6px;color:#64748B;cursor:pointer;font-weight:600;margin-top:8px;font-family:Heebo,sans-serif">+ סיבת אי ביצוע</button>';
          }
          html+='<div id="dr-reason-'+openLineId+'-'+realIdx+'" style="display:none;background:#FFFBEB;border:1px solid rgba(234,179,8,.35);border-radius:9px;padding:10px 12px;margin-top:6px">';
          html+='<div style="font-size:11px;font-weight:700;color:#854D0E;margin-bottom:7px">סיבת אי ביצוע</div>';
          html+='<select id="dr-sel-'+openLineId+'-'+realIdx+'" style="width:100%;background:#fff;border:1px solid rgba(234,179,8,.4);border-radius:7px;padding:7px 10px;color:#1E293B;font-size:13px;font-family:Heebo,sans-serif;margin-bottom:7px">';
          var reasons2=['בחר סיבה...','מחסן','ליקוט','סט-אפ','הנדסה','פיתוח','ייצור','איכות','IT','תכנון','שיווק','אחר'];
          reasons2.forEach(function(opt){html+='<option value="'+opt+'"'+(r.rowReason===opt?' selected':'')+'>'+opt+'</option>';});
          html+='</select>';
          html+='<textarea id="dr-note-'+openLineId+'-'+realIdx+'" placeholder="פרט / הערה נוספת..." style="width:100%;background:#fff;border:1px solid rgba(234,179,8,.4);border-radius:7px;padding:7px 10px;color:#1E293B;font-size:13px;font-family:Heebo,sans-serif;resize:none;line-height:1.5;margin-bottom:7px" rows="2">'+(r.rowNote||'')+'</textarea>';
          html+='<div style="display:flex;gap:6px">';
          html+='<button data-lid="'+openLineId+'" data-idx="'+realIdx+'" onclick="saveDashRowReason(this.dataset.lid,parseInt(this.dataset.idx))" style="flex:1;padding:7px;background:var(--green);border:none;border-radius:7px;color:#fff;font-size:13px;font-weight:700;cursor:pointer">שמור</button>';
          html+='<button data-lid="'+openLineId+'" data-idx="'+realIdx+'" onclick="toggleDashRowReason(this.dataset.lid,parseInt(this.dataset.idx))" style="padding:7px 12px;background:#fff;border:1px solid var(--border);border-radius:7px;color:var(--dim);font-size:13px;cursor:pointer">ביטול</button>';
          html+='</div></div>';
          if(!isDone&&!isPartial){
            html+='<div id="dr-pinput-'+realIdx+'" style="display:none;align-items:center;gap:8px;margin-top:8px">';
            html+='<input type="number" inputmode="numeric" placeholder="כמה?" min="1" max="'+r.qty+'" id="dr-partial-'+realIdx+'" style="flex:1;background:var(--surface);border:1px solid var(--border);border-radius:8px;padding:7px 12px;color:var(--text);font-size:18px;font-family:var(--mono);text-align:center">';
            html+='<button data-lid="'+openLineId+'" data-idx="'+realIdx+'" onclick="markDashPartial(this.dataset.lid,parseInt(this.dataset.idx))" style="padding:7px 16px;background:var(--yellow);border:none;border-radius:8px;color:#fff;font-size:14px;font-weight:700;cursor:pointer">שמור</button>';
            html+='</div>';
          }
        }
        html+='</div>';
