...
function renderDash(){
  ...
  calcs.map(function(c){
    ...
    var redRows2=rows.filter(function(r){return r.isRed&&r.status!=='done';});
    var workRows=rows.filter(function(r){return !r.isRed&&r.status!=='done';});
    var doneRows=rows.filter(function(r){return r.status==='done';});
    var redOpen=redRows2.length;
    // Row summary strips
    var strips='';
    if(redOpen>0) strips+='<div style="display:flex;align-items:center;gap:4px;background:#FFF0F0;border:1px solid rgba(220,38,38,.25);border-radius:6px;padding:3px 7px;margin-bottom:3px"><span style="width:6px;height:6px;border-radius:50%;background:#DC2626;flex-shrink:0;display:inline-block"></span><span style="font-size:10px;color:#DC2626;font-weight:700">'+redOpen+' אדומות</span></div>';
    if(workRows.length>0) strips+='<div style="display:flex;align-items:center;gap:4px;background:#FFFBEB;border:1px solid rgba(217,119,6,.25);border-radius:6px;padding:3px 7px;margin-bottom:3px"><span style="width:6px;height:6px;border-radius:50%;background:#D97706;flex-shrink:0;display:inline-block"></span><span style="font-size:10px;color:#D97706;font-weight:700">'+workRows.length+' בעבודה</span></div>';
    if(doneRows.length>0) strips+='<div style="display:flex;align-items:center;gap:4px;background:#F0FDF4;border:1px solid rgba(5,150,105,.25);border-radius:6px;padding:3px 7px;margin-bottom:3px"><span style="width:6px;height:6px;border-radius:50%;background:#059669;flex-shrink:0;display:inline-block"></span><span style="font-size:10px;color:#059669;font-weight:700">'+doneRows.length+' בוצעו</span></div>';
    return '<div data-lid2="'+c.id+'" style="background:var(--card);border:1px solid var(--border);border-radius:12px;padding:10px 8px;cursor:pointer;text-align:center" onclick="openLineDetail(this.getAttribute(\'data-lid2\'))">'+
