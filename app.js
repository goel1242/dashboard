...
function loadState(){
  ...
  try{
    ...
    // ops_history loaded separately in renderHistory
    ...
    var dr=store.getItem('ops_delay_reasons');
    if(dr) delayReasons=JSON.parse(dr);
  }catch(e){}
}
function saveState(){
...
function clearDay(){
  // Archive open pallets to history before clearing
  if(pallets.length){
    try{
      var ph=JSON.parse(store.getItem('ops_pallet_history')||'[]');
      pallets.forEach(function(p){
        if(!p.closed){
          p.archivedAt=new Date().toLocaleDateString('he-IL');
          p.closedDate=p.closedDate||new Date().toLocaleDateString('he-IL');
        }
  ...
  store.removeItem('ops_order_rows');saveState();syncToServer();
}
function saveToHist(dateStr){
  try{
    ...
    // Also sync history to Supabase
    var histRows=[{key:'history',value:JSON.stringify(h),updated_at:new Date().toISOString()}];
    sbFetch('ops_data',{method:'POST',prefer:'resolution=merge-duplicates',body:JSON.stringify(histRows)}).catch(function(){});
  }catch(e){}
}
...
function syncToServer(){
  var payload={
    ...
    order_rows:JSON.stringify(orderRows)
  };
  ...
  sbFetch('ops_data?select=key,value').then(function(rows){
    ...
    if(map.history){
      try{
        ...
        var localHist=JSON.parse(store.getItem('ops_history')||'{}');
        var localKeys=Object.keys(localHist).length;
        var remoteKeys=Object.keys(remoteHist).length;
        // Merge: keep all keys from both, remote wins on conflict
        if(remoteKeys>0){
          var merged=Object.assign({},localHist,remoteHist);
          var mKeys=Object.keys(merged).sort().reverse();
          if(mKeys.length>35) mKeys.slice(35).forEach(function(k){delete merged[k];});
          store.setItem('ops_history',JSON.stringify(merged));
        }
      }catch(e){}
    }
    if(map.order_rows){
      var syncedRows=JSON.parse(map.order_rows);
      // Only overwrite if synced data has content OR local is empty
      var localCount=Object.values(orderRows).reduce(function(s,a){return s+a.length;},0);
      var syncCount=Object.values(syncedRows).reduce(function(s,a){return s+a.length;},0);
      if(syncCount>0||localCount===0){orderRows=syncedRows;recalcRedRows();}
    }
...
function renderProductivity(){
  ...
  var data=LINES.map(function(l){
    ...
    // זמן ביצוע: use work plan mins/unit if available, else actual rate
  ...
  html+=data.map(function(d){
    ...
    return '<div style="background:var(--card);border:1px solid var(--border);border-radius:12px;padding:12px 14px;margin-bottom:8px;box-shadow:0 1px 3px rgba(0,0,0,.06)">' +
      ...
      '<div class="sgrid3">' +
        ...
        '<div class="sbox"><div class="sbox-v" style="color:'+(d.etaOver?'var(--red)':'var(--green)')+'">'+d.etaStr+'</div><div class="sbox-l">זמן ביצוע סיום</div></div>' +
      '</div>' +
...
function buildStaffingRecommendation(data){
  ...
  issues.forEach(function(d){
    ...
    if(donor){
      html+='<div style="font-size:12px;color:var(--text);margin-bottom:5px">העבר עובד מ-<strong>'+donor.name+'</strong> ל-<strong>'+d.name+'</strong> ('+msg+'זמן ביצוע '+d.etaStr+')</div>';
    } else {
...
function addPallet(){
  var id='P'+Date.now();
  var now=new Date();pallets.push({id:id,orderId:'',customer:'',pkNum:'',shipType:'sea',maxHeight:220,special:false,specialNote:'',instructions:'',cartons:{dolbox:0,upgrade:0,box2x2:0},sourceLogs:[],closed:false,createdAt:nowTime(),createdDate:now.toLocaleDateString('he-IL'),createdDateObj:now.toISOString()});
  saveState();syncToServer();renderPallets();buildRequestForms();
}
var _palletSaveTimer=null;
function palletField(el){
  ...
  if(field==='shipType'){
    p.shipType=el.value;
    p.maxHeight=el.value==='air'?160:220;
    var dc=el.value==='air'?'var(--blue)':'var(--green)';
    var dotEl=document.getElementById('pdot-'+id);
...
function debouncePalletSave(){
  if(_palletSaveTimer) clearTimeout(_palletSaveTimer);
  _palletSaveTimer=setTimeout(function(){
    saveState();syncToServer();
    _palletSaveTimer=null;
  },1500);
}
function closePallet(el){
  var id=el.getAttribute('data-pid'),p=pallets.filter(function(x){return x.id===id;})[0];
  if(!p) return;
  p.closed=true;p.closedAt=nowTime();p.closedDate=new Date().toLocaleDateString('he-IL');
  // Save to pallet history
  try{
    var ph=JSON.parse(store.getItem('ops_pallet_history')||'[]');
    ph.unshift(JSON.parse(JSON.stringify(p)));
    if(ph.length>200) ph=ph.slice(0,200);
    store.setItem('ops_pallet_history',JSON.stringify(ph));
  }catch(e){}
  saveState();syncToServer();renderPallets();buildRequestForms();
...
function renderPallets(){
  ...
  var focusedVal=document.activeElement?document.activeElement.value:'';
  var focusedPos=document.activeElement&&document.activeElement.selectionStart!==undefined?document.activeElement.selectionStart:0;
  ...
  pallets.forEach(function(p){
    ...
    // Cartons display - pills style
    html+='<div style="border-top:1px solid var(--border);padding-top:10px;margin-bottom:8px">';
    html+='<div style="font-size:11px;color:var(--dim);margin-bottom:6px">סוגי קרטונים על המשטח</div>';
    var cartonColors={dolbox:{bg:'#EFF6FF',col:'#1D4ED8',border:'#BFDBFE'},upgrade:{bg:'#F0FDF4',col:'#166534',border:'#BBF7D0'},box2x2:{bg:'#FFF7ED',col:'#9A3412',border:'#FED7AA'}};
    var hasCartons=false;
    html+='<div style="display:flex;gap:6px;flex-wrap:wrap;margin-bottom:8px">';
    CTYPES.forEach(function(ct){
      var qty=p.cartons[ct.id]||0;
      if(qty<=0) return;
      hasCartons=true;
      var colors=cartonColors[ct.id]||{bg:'var(--surface)',col:'var(--text)',border:'var(--border)'};
      html+='<div style="display:flex;align-items:center;gap:6px;padding:5px 12px;border-radius:8px;background:'+colors.bg+';border:1px solid '+colors.border+'">';
      html+='<span style="font-size:12px;font-weight:600;color:'+colors.col+'">'+ct.name+'</span>';
      html+='<span style="font-size:16px;font-weight:800;font-family:var(--mono);color:'+colors.col+'">'+qty+'</span>';
      html+='</div>';
    });
    if(!hasCartons) html+='<span style="font-size:12px;color:var(--faint)">אין קרטונים עדיין</span>';
    html+='</div>';
    // Stats row
    html+='<div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:6px;margin-bottom:6px">';
    html+='<div style="text-align:center"><div style="font-size:18px;font-weight:800;font-family:var(--mono);color:var(--text)">'+calc.tot+'</div><div style="font-size:10px;color:var(--dim)">קרטונים</div></div>';
    html+='<div style="text-align:center"><div style="font-size:18px;font-weight:800;font-family:var(--mono);color:'+cmColor+'">'+calc.totalCm+'</div><div style="font-size:10px;color:var(--dim)">ס"מ גובה</div></div>';
    html+='<div style="text-align:center"><div style="font-size:18px;font-weight:800;font-family:var(--mono);color:var(--dim)">'+p.maxHeight+'</div><div style="font-size:10px;color:var(--dim)">מקסימום</div></div>';
    html+='</div>';
    // Height progress bar
    var heightPct=p.maxHeight>0?Math.min(Math.round(calc.totalCm/p.maxHeight*100),100):0;
    var almostFull=calc.totalCm>p.maxHeight*.85&&calc.tot>0;
    html+='<div style="display:flex;justify-content:space-between;font-size:10px;color:var(--dim);margin-bottom:3px">';
    html+='<span>גובה משטח</span>';
    html+='<span style="color:'+cmColor+';font-weight:700">'+calc.totalCm+' / '+p.maxHeight+' ס"מ ('+heightPct+'%)'+(almostFull?' — כמעט מלא!':'')+'</span>';
    html+='</div>';
    html+='<div style="height:6px;background:var(--border);border-radius:3px;overflow:hidden;margin-bottom:8px">';
    html+='<div style="height:100%;background:'+cmColor+';width:'+heightPct+'%;transition:width .3s"></div></div>';
    html+='</div>';
    // Height summary (keep for compatibility)
    html+='<div id="psum-'+p.id+'" style="display:none">'+calc.tot+' קרטונים | '+calc.layers+' קומות | '+calc.totalCm+'/'+p.maxHeight+' cm</div>';
...
function renderHistory(){
  ...
  var histData={};
  try{histData=JSON.parse(store.getItem('ops_history')||'{}');}catch(e){}
  var keys=Object.keys(histData).sort().reverse();
  ...
  var chartDays=filtered.slice(0,7).reverse();
  var chartHtml='<div style="background:var(--card);border:1px solid var(--border);border-radius:12px;padding:14px;margin-bottom:14px">';
  chartHtml+='<div style="font-size:13px;font-weight:700;margin-bottom:10px">ביצוע שבועי (%)</div>';
  chartHtml+='<div style="display:flex;align-items:flex-end;gap:6px;height:80px">';
  chartDays.forEach(function(dk){
    var d2=histData[dk],totT=0,totD=0;
    LINES.forEach(function(l){totT+=(d2.targets[l.id]||0);var reps=d2.reports[l.id]||[];var last=reps[reps.length-1];totD+=last?last.done:0;});
    var ach=totT>0?Math.round(totD/totT*100):0;
    var barH=Math.max(4,Math.round(ach*0.75));
    var barCol=ach>=95?'var(--green)':ach>=85?'var(--yellow)':'var(--red)';
    var dayN=DAYS[new Date(dk).getDay()];
    chartHtml+='<div style="flex:1;display:flex;flex-direction:column;align-items:center;gap:3px">';
    chartHtml+='<div style="font-size:10px;font-weight:700;color:'+barCol+'">'+ach+'%</div>';
    chartHtml+='<div style="width:100%;height:'+barH+'px;background:'+barCol+';border-radius:3px 3px 0 0;max-width:36px"></div>';
    chartHtml+='<div style="font-size:10px;color:var(--dim)">'+dayN+'</div>';
    chartHtml+='</div>';
  });
  chartHtml+='</div></div>';

  el2.innerHTML=chartHtml+filtered.map(function(dk){
    var d2=histData[dk],dayName=DAYS[new Date(dk).getDay()],dateStr=new Date(dk).toLocaleDateString('he-IL');
...
function genShiftSummary(){
  ...
  lines2.push('');
  var openPallets=pallets.filter(function(p){return !p.closed;}).length;
  lines2.push('משטחים שנפתחו: '+(closedPallets+openPallets)+' | נסגרו: '+closedPallets+' | פתוחים: '+openPallets);
...
function syncMgrShiftSel(){
  var sel=document.getElementById('mgr-shift-sel');
  if(sel) sel.value=currentShiftType;
  // Also update shift pill display
  var shObj=SHIFT_TYPES[currentShiftType]||SHIFT_TYPES['regular'];
  var pill=document.getElementById('shift-pill');
  if(pill) pill.textContent=shObj.times+' - '+shObj.hours+'sh ('+shObj.brk+'m)';
}
...
function showMatSkuSuggestions(){
  var lineEl=document.getElementById('req-line');
...
function parseWorkPlan(rows){
  ...
  rows.forEach(function(row){
    ...
    if(qty>0){
        // Normalize start date to YYYY-MM-DD for comparison
      ...
      if(s.indexOf('T')>=0){
        startNorm=s.substring(0,10); // ISO: 2026-04-19T07:48:00.000Z
      } else if(s.match(/^\d{4}-\d{2}-\d{2}/)){
        startNorm=s.substring(0,10); // YYYY-MM-DD
      } else if(s.match(/^\d{2}\/\d{2}\/\d{4}/)){
        var p=s.substring(0,10).split('/');
        startNorm=p[2]+'-'+p[1]+'-'+p[0]; // DD/MM/YYYY
      } else if(s.match(/^\d{1,2}\/\d{1,2}\/\d{4}/)){
        var p2=s.split(' ')[0].split('/');
        if(p2.length===3) startNorm=p2[2]+'-'+('0'+p2[1]).slice(-2)+'-'+('0'+p2[0]).slice(-2);
      } else if(s.match(/^\d{4}\//)){
        var p3=s.split(' ')[0].split('/');
        if(p3.length===3) startNorm=p3[0]+'-'+('0'+p3[1]).slice(-2)+'-'+('0'+p3[2]).slice(-2); // YYYY/MM/DD
      }
      if(!startNorm&&startStr){
        // Fallback: try Date parse
        var d=new Date(startStr);
        if(!isNaN(d)) startNorm=d.getFullYear()+'-'+('0'+(d.getMonth()+1)).slice(-2)+'-'+('0'+d.getDate()).slice(-2);
      }
...
function recalcRedRows(){
  ...
  Object.keys(orderRows).forEach(function(lineId){
    (orderRows[lineId]||[]).forEach(function(r){
      ...
      else if(sd){var d2=new Date(sd);if(!isNaN(d2))norm=d2.getFullYear()+'-'+('0'+(d2.getMonth()+1)).slice(-2)+'-'+('0'+d2.getDate()).slice(-2);}
...
function saveOrderRows(){
  ...
  // order_rows saved via syncToServer
}
...
function renderLineRows(){
  ...
  sorts.forEach(function(s){
    ...
    html+='<button onclick="window._ltSortMode=\''+s[0]+'\';renderLineRows()" style="font-size:11px;padding:4px 10px;border-radius:16px;border:1px solid '+(active?'var(--blue)':'var(--border)')+';background:'+(active?'var(--blue)':'var(--surface)')+';color:'+(active?'#fff':'var(--dim)')+';cursor:pointer;font-family:var(--font)">'+(active?'&#10003; ':'')+s[1]+'</button>';
  });
  html+='</div>';
  // Search
  html+='<input type="search" id="lt-search" autocomplete="off" autocorrect="off" autocapitalize="off" spellcheck="false" inputmode="text" autocomplete="off" autocorrect="off" autocapitalize="off" spellcheck="false" placeholder="חפש הזמנה / מק-ט / לקוח..." autocomplete=\"off\" autocorrect=\"off\" autocapitalize=\"off\" spellcheck=\"false\" oninput="renderLineRows()" value="'+search+'" style="width:100%;background:var(--surface);border:1px solid var(--border);border-radius:8px;padding:8px 12px;color:var(--text);font-size:13px;margin-bottom:8px">';
  ...
  filtered.forEach(function(r){
    ...
    var rowBg=isDone?'#F0FDF4':isPartial?'#FFF8F0':isRed?'#FFF0F0':'var(--card)';
    var borderCol=isRed?'rgba(239,68,68,.4)':isPartial?'rgba(234,88,12,.4)':isDone?'rgba(34,197,94,.35)':'var(--border)';
    var borderW=isRed||isPartial||isDone?'1.5px':'1px';
    var textCol='#0F172A';
    var subCol='#334155';
    var orderCol=isRed?'#DC2626':isPartial?'#EA580C':isDone?'#16A34A':'var(--blue)';
    var dimCol='#64748B';

    html+='<div style="background:'+rowBg+';border:'+borderW+' solid '+borderCol+';border-radius:12px;padding:12px 14px;margin-bottom:8px">';
    html+='<div style="display:flex;justify-content:space-between;align-items:flex-start;gap:10px">';

    // Left side
    html+='<div style="flex:1;min-width:0">';
    // Customer name above order
    if(r.customer) html+='<div style="font-size:16px;font-weight:800;color:#0F172A;margin-bottom:3px">'+r.customer+'</div>';
    // Order + line
    if(r.order) html+='<div style="display:flex;align-items:baseline;gap:6px;margin-bottom:4px"><span style="font-size:14px;font-weight:600;color:'+orderCol+'">'+r.order+'</span><span style="font-size:11px;color:'+dimCol+'">/ שורה '+(r.orderLine||'')+'</span></div>';
    // Desc
    if(r.desc) html+='<div style="font-size:14px;font-weight:600;color:'+subCol+';margin-bottom:6px;line-height:1.3">'+r.desc+'</div>';
    // SKU badge
    if(r.sku){
      var skuAccent=isRed?'rgba(239,68,68,.25)':isPartial?'rgba(234,88,12,.2)':'var(--border)';
      var skuLabel=isRed?'#DC2626':isPartial?'#EA580C':'#64748B';
      html+='<div style="background:#fff;border:1px solid '+skuAccent+';border-radius:7px;padding:4px 8px;display:inline-block;margin-bottom:4px">';
      html+='<div style="font-size:9px;color:'+skuLabel+';font-weight:700;margin-bottom:1px">מק"ט</div>';
      html+='<div style="font-size:14px;font-weight:700;font-family:var(--mono);color:#1E293B">'+r.sku+'</div>';
      html+='</div>';
    }
    // skuB badge
    if(showSkuB&&r.skuB){
      html+='<div style="background:#EFF6FF;border:1px solid #BFDBFE;border-radius:7px;padding:4px 8px;display:inline-block;margin-right:4px">';
      html+='<div style="font-size:10px;color:var(--blue);margin-bottom:1px">מק"ט בן</div>';
      html+='<div style="font-size:15px;font-weight:700;font-family:var(--mono);color:var(--blue)">'+r.skuB+'</div>';
      html+='</div>';
    }
    html+='</div>';
    ...
    if(showExpiry&&r.expiry){
      var expiryAccent=isRed?'rgba(239,68,68,.5)':isPartial?'rgba(234,88,12,.4)':'var(--border)';
      var expiryLbl=isRed?'#DC2626':isPartial?'#EA580C':'#64748B';
      html+='<div style="background:#fff;border:1.5px solid '+expiryAccent+';border-radius:8px;padding:4px 10px;text-align:center">';
      html+='<div style="font-size:9px;color:'+expiryLbl+';font-weight:700;letter-spacing:.5px">פק"ע</div>';
      html+='<div style="font-size:14px;font-weight:900;font-family:var(--mono);color:'+expiryLbl+'">'+r.expiry+'</div>';
      if(isRed) html+='<div style="font-size:9px;color:#DC2626;font-weight:700">⚠ עבר</div>';
      html+='</div>';
    }
    if(isDone||isPartial){
      var doneCol=isDone?'#16A34A':'#EA580C';
    ...
    if(isRed&&!isDone){
      var hasReason=r.delayReason;
      if(hasReason){
        html+='<div style="background:var(--green-bg);border-radius:6px;padding:6px 10px;margin-top:6px;border:1px solid rgba(34,197,94,.2)">';
        html+='<div style="display:flex;justify-content:space-between;align-items:center">';
        html+='<span style="font-size:11px;font-weight:700;color:var(--green)">סיבה: '+r.delayReason+'</span>';
        ...
        html+='</div>';
        if(r.missingsku) html+='<div style="font-size:11px;font-family:var(--mono);color:var(--text);margin-top:2px">מק"ט חסר: '+r.missingsku+'</div>';
        if(r.delaynote) html+='<div style="font-size:11px;color:var(--dim);margin-top:1px">'+r.delaynote+'</div>';
        html+='</div>';
      } else {
        html+='<button data-lid="'+currentLine+'" data-idx="'+realIdx+'" onclick="toggleRedRowReason(this.dataset.lid,parseInt(this.dataset.idx))" style="width:100%;font-size:11px;padding:5px;background:#FEF9C3;border:1px solid rgba(234,179,8,.4);border-radius:6px;color:#854D0E;cursor:pointer;font-weight:600;margin-top:6px;font-family:Heebo,sans-serif">+ הוסף סיבה לעיכוב</button>';
      }
      ...
      html+='<div style="font-size:11px;font-weight:700;color:#854D0E;margin-bottom:6px">סיבת עיכוב</div>';
      html+='<select id="rr-sel-'+currentLine+'-'+realIdx+'" style="width:100%;background:var(--surface);border:1px solid var(--border);border-radius:7px;padding:6px 10px;color:var(--text);font-size:13px;font-family:Heebo,sans-serif;margin-bottom:6px">';
      delayReasons.forEach(function(opt){
        html+='<option value="'+opt+'"'+(r.delayReason===opt?' selected':'')+'>'+opt+'</option>';
      });
      html+='<option value="אחר"'+(r.delayReason==='אחר'?' selected':'')+'>אחר</option>';
      html+='</select>';
      html+='<input type="text" inputmode="text" autocomplete="off" autocorrect="off" autocapitalize="off" spellcheck="false" id="rr-sku-'+currentLine+'-'+realIdx+'" placeholder="מק"ט חסר..." autocomplete="off" autocapitalize="off" value="'+(r.missingsku||'')+'" style="width:100%;background:var(--surface);border:1px solid var(--border);border-radius:7px;padding:6px 10px;color:var(--text);font-size:13px;font-family:var(--mono);margin-bottom:6px">';
      html+='<input type="text" id="rr-note-'+currentLine+'-'+realIdx+'" placeholder="הערה..." autocomplete="off" value="'+(r.delaynote||'')+'" style="width:100%;background:var(--surface);border:1px solid var(--border);border-radius:7px;padding:6px 10px;color:var(--text);font-size:13px;margin-bottom:8px">';
      html+='<div style="display:flex;gap:6px">';
    ...
    if(!isDone){
      ...
      html+='<input type="number" inputmode="numeric" id="lt-partial-'+realIdx+'" inputmode="numeric" placeholder="כמה?" min="1" max="'+r.qty+'" style="flex:1;background:var(--surface);border:1px solid var(--border);border-radius:8px;padding:7px 12px;color:var(--text);font-size:18px;font-family:var(--mono);text-align:center">';
  ...
  // Restore focus and cursor after rebuild
...
function renderRedRowsManager(){
  ...
  LINES.forEach(function(l){
    ...
    rows.forEach(function(r,idx){
      if(r.isRed&&r.status!=='done'){
        all.push({line:l,row:r,idx:idx});
      }
    });
  });
  var hasOrderRows=Object.keys(orderRows).some(function(k){return (orderRows[k]||[]).length>0;});
  if(!all.length){
    el.innerHTML=hasOrderRows?'<div class="empty">אין שורות אדומות פתוחות</div>':'<div class="empty">טען תוכנית עבודה תחילה</div>';
    return;
  }
  // Group by line
  var byLine={};
  all.forEach(function(item){
    var lid=item.line.id;
    if(!byLine[lid]) byLine[lid]={line:item.line,rows:[]};
    byLine[lid].rows.push(item);
  });
  ...
  Object.values(byLine).forEach(function(group){
    ...
    html+='<div style="font-size:13px;font-weight:700;color:var(--text);padding:6px 10px;background:var(--red-bg);border-radius:8px 8px 0 0;border:1px solid rgba(220,38,38,.2)">'+
      group.line.name+' — '+group.rows.length+' שורות</div>';
    group.rows.forEach(function(item){
      var r=item.row;
      html+='<div style="padding:10px 12px;background:var(--card);border:1px solid var(--border);border-top:none;last-child:border-radius:0 0 8px 8px">';
      html+='<div style="display:flex;justify-content:space-between;align-items:flex-start">';
      html+='<div>';
      if(r.order) html+='<div style="font-size:14px;font-weight:700;color:var(--red)">'+r.order+(r.orderLine?' / '+r.orderLine:'')+'</div>';
      if(r.desc) html+='<div style="font-size:13px;font-weight:600;color:var(--text);margin-top:2px">'+r.desc+'</div>';
      if(r.sku) html+='<div style="font-size:12px;font-family:var(--mono);color:var(--dim)">'+r.sku+'</div>';
      if(r.customer) html+='<div style="font-size:12px;color:var(--dim)">'+r.customer+'</div>';
      html+='</div>';
      html+='<div style="text-align:left;flex-shrink:0">';
      if(r.expiry) html+='<div style="font-size:11px;color:var(--red);padding:2px 7px;background:var(--red-bg);border-radius:5px;margin-bottom:4px">'+r.expiry+' ⚠</div>';
      html+='<div style="font-size:18px;font-weight:800;color:var(--red);font-family:var(--mono)">'+r.qty+'</div>';
      html+='<div style="font-size:10px;color:var(--dim)">יח לאריזה</div>';
      html+='</div></div>';
      html+='</div>';
    });
    html+='</div>';
...
function noStockMaterial(i){
  ...
  req.endTs=Date.now();
  // Stop timer
  if(req.timerInterval){clearInterval(req.timerInterval);req.timerInterval=null;}
  saveMaterials();
  renderMaterials();
  showT('סומן אין מלאי');
}
...
function renderMatHistory(){
  ...
  Object.keys(groups).forEach(function(dk){
    ...
    groups[dk].forEach(function(req){
      var i=materialRequests.indexOf(req);
...
// SKU CATALOG
// ============================================================
var skuCatalog = []; // loaded from sku_catalog.js

function loadSkuCatalog(){
  // Always reload from global vars
  ...
  if(skuCatalog.length){
    loadCatalogEdits();
    console.log('SKU catalog loaded:',skuCatalog.length,'items');
  } else {
    // Retry - scripts might still be loading
    setTimeout(function(){
      ...
      else console.warn('SKU catalog not loaded - check sku_catalog_1.js and sku_catalog_2.js');
    },1000);
  }
}
...
function skuLookup(sku){
  ...
  return skuCatalog.find(function(item){
    return item.sku.toUpperCase() === s;
  }) || null;
}

function skuSearch(query, lineId){
  if(!query || query.length < 2) return [];
  var q = query.toLowerCase();
  var results = skuCatalog.filter(function(item){
    var matchLine = !lineId || !item.lineId || item.lineId === lineId;
    return matchLine && (
...
function renderCatalogSuggestions(inputId, lineId){
  var inp = document.getElementById(inputId);
  var box = document.getElementById(inputId + '-suggestions');
  if(!inp || !box) return;
  var val = inp.value.trim();
  if(val.length < 2){box.style.display='none';return;}
  var results = skuSearch(val, lineId);
  if(!results.length){box.style.display='none';return;}
  box.innerHTML = results.map(function(r){
    return '<div onclick="selectCatalogItem(\'' + inputId + '\',\'' + r.sku.replace(/'/g,"\'") + '\',\'' + r.desc.replace(/'/g,"\'").replace(/"/g,"&quot;") + '\')" style="padding:8px 12px;cursor:pointer;border-bottom:1px solid var(--border);font-size:13px">' +
      '<div style="font-weight:700;font-family:var(--mono)">' + r.sku + '</div>' +
      '<div style="font-size:11px;color:var(--dim)">' + r.desc + (r.lineId?' | ' + (r.lineId==='pick'?'\u05dc\u05d9\u05e7\u05d5\u05d8':r.lineId==='drive'?'\u05d9\u05d7\u05d9\u05d3\u05d5\u05ea \u05d4\u05e0\u05e2\u05d4':r.lineId==='kit'?'\u05e2\u05e8\u05db\u05d5\u05ea':r.lineId==='cable'?'\u05db\u05d1\u05dc\u05d9\u05dd':'\u05e1\u05e4\u05e7\u05d9 \u05db\u05d5\u05d7'):'') + '</div>' +
    '</div>';
  }).join('');
  box.style.display = 'block';
}

function selectCatalogItem(inputId, sku, desc){
  var inp = document.getElementById(inputId);
  var box = document.getElementById(inputId + '-suggestions');
  if(inp) inp.value = sku;
  if(box) box.style.display = 'none';
  // Fill desc if there's a desc field
  var descInp = document.getElementById(inputId + '-desc');
...
function renderCatalogStats(){
  ...
  var html='<div style="font-size:12px;font-weight:700;color:var(--dim);margin-bottom:8px">'+skuCatalog.length+' \u05de\u05e7"\u05d8\u05d9\u05dd \u05e4\u05e2\u05d9\u05dc\u05d9\u05dd \u05d1\u05e7\u05d8\u05dc\u05d5\u05d2</div>';
...
function calcLineETA(lineId){
  ...
  openRows.forEach(function(r){
    ...
    if(cat && cat.duration > 0){
      totalSeconds += cat.duration * remaining;
    } else {
...
function renderLineETAs(){
  ...
  var elapsedH = elapsed();
  var shiftLeft = Math.max(0.1, shiftH - elapsedH);

  // Worker editor
  var workerHtml = '<div style="background:var(--card);border:1px solid var(--border);border-radius:10px;padding:10px 14px;margin-bottom:10px">';
  workerHtml += '<div style="font-size:12px;font-weight:700;color:var(--dim);margin-bottom:8px">כוח אדם לחישוב</div>';
  workerHtml += '<div style="display:grid;grid-template-columns:repeat(5,1fr);gap:8px">';
  LINES.forEach(function(l){
    var w = DEFAULT_WORKERS[l.id] || 1;
    workerHtml += '<div style="text-align:center">';
    workerHtml += '<div style="font-size:11px;color:var(--dim);margin-bottom:3px">' + l.name + '</div>';
    workerHtml += '<input type="number" min="1" max="20" value="' + w + '" data-lid="' + l.id + '" onchange="DEFAULT_WORKERS[this.dataset.lid]=parseInt(this.value)||1;saveState();renderLineETAs()" style="width:100%;text-align:center;background:var(--surface);border:1px solid var(--border);border-radius:6px;padding:4px;font-size:16px;font-weight:700;font-family:var(--mono);color:var(--text)">';
    workerHtml += '</div>';
  });
  workerHtml += '</div></div>';
  ...
  LINES.forEach(function(l){
    ...
    var totalHours = totalSeconds / 3600;
    var adjustedHours = totalHours / Math.max(1, workers);
    var coveragePct = totalQty > 0 ? Math.round(coveredQty / totalQty * 100) : 0;

    // Workers needed to finish on time
    var workersNeeded = shiftLeft > 0 ? Math.ceil(totalHours / shiftLeft) : workers;
    var onTime = adjustedHours <= shiftLeft;
    var overBy = onTime ? 0 : (adjustedHours - shiftLeft).toFixed(1);

    var col = onTime ? 'var(--green)' : 'var(--red)';
    var statusLabel = onTime ? 'יסיים בזמן' : 'חריגה: +' + overBy + ' שע';
    var statusBg = onTime ? 'var(--green-bg)' : 'var(--red-bg)';

    lineCards += '<div style="background:var(--card);border:1px solid var(--border);border-radius:12px;padding:12px 14px;margin-bottom:8px">';
    // Header
    lineCards += '<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:10px">';
    lineCards += '<span style="font-size:15px;font-weight:800">' + l.name + '</span>';
    lineCards += '<span style="font-size:12px;padding:3px 12px;background:' + statusBg + ';color:' + col + ';border-radius:10px;font-weight:700">' + statusLabel + '</span>';
    lineCards += '</div>';

    // Main numbers
    lineCards += '<div style="display:grid;grid-template-columns:repeat(3,1fr);gap:6px;margin-bottom:10px">';
    // Total TT
    lineCards += '<div style="text-align:center;background:var(--surface);border-radius:8px;padding:8px">';
    lineCards += '<div style="font-size:22px;font-weight:900;font-family:var(--mono);color:var(--blue)">' + totalHours.toFixed(1) + '</div>';
    lineCards += '<div style="font-size:10px;color:var(--dim)">שעות TT כולל</div></div>';
    // With workers
    lineCards += '<div style="text-align:center;background:var(--surface);border-radius:8px;padding:8px">';
    lineCards += '<div style="font-size:22px;font-weight:900;font-family:var(--mono);color:' + col + '">' + adjustedHours.toFixed(1) + '</div>';
    lineCards += '<div style="font-size:10px;color:var(--dim)">' + workers + ' עובדים — שעות</div></div>';
    // Shift left
    lineCards += '<div style="text-align:center;background:var(--surface);border-radius:8px;padding:8px">';
    lineCards += '<div style="font-size:22px;font-weight:900;font-family:var(--mono);color:var(--faint)">' + shiftLeft.toFixed(1) + '</div>';
    lineCards += '<div style="font-size:10px;color:var(--dim)">שעות שנותרו</div></div>';
    lineCards += '</div>';

    // Workers needed analysis
    if(!onTime){
      lineCards += '<div style="background:var(--red-bg);border:1px solid rgba(220,38,38,.2);border-radius:8px;padding:8px 12px;margin-bottom:8px;font-size:12px;color:var(--red)">';
      lineCards += 'לסיום בזמן נדרשים <strong>' + workersNeeded + ' עובדים</strong> (כרגע ' + workers + ')';
      lineCards += '</div>';
    } else if(workers > 1) {
      var minWorkers = Math.ceil(totalHours / shiftLeft);
      if(minWorkers < workers){
        lineCards += '<div style="background:var(--green-bg);border:1px solid rgba(5,150,105,.2);border-radius:8px;padding:8px 12px;margin-bottom:8px;font-size:12px;color:var(--green)">';
        lineCards += 'ניתן לסיים עם <strong>' + minWorkers + ' עובדים</strong> בלבד';
        lineCards += '</div>';
      }
    }

    // Coverage bar
    lineCards += '<div style="display:flex;justify-content:space-between;font-size:10px;color:var(--dim);margin-bottom:3px">';
    lineCards += '<span>כיסוי קטלוג TT</span>';
    lineCards += '<span style="font-weight:700;color:' + (coveragePct>=80?'var(--green)':'var(--yellow)') + '">' + coveragePct + '% מהיחידות</span>';
    lineCards += '</div>';
    lineCards += '<div style="height:4px;background:var(--border);border-radius:2px;overflow:hidden">';
    lineCards += '<div style="height:100%;background:' + (coveragePct>=80?'var(--green)':'var(--yellow)') + ';width:' + coveragePct + '%"></div></div>';
    if(unknownQty > 0){
      lineCards += '<div style="font-size:10px;color:var(--yellow);margin-top:4px">' + unknownQty + ' יח ללא נתוני TT בקטלוג</div>';
    }
    lineCards += '</div>';
  });

  if(!hasData){
    el.innerHTML = workerHtml + '<div class="empty">טען תוכנית עבודה תחילה</div>';
  } else {
    el.innerHTML = workerHtml + lineCards;
  }
}
...
function removeCatalogEdit(idx){
  ...
  // Remove from live catalog only if it was a custom item
...
function renderFridayMode(){
  var el = document.getElementById('lines-list');
  if(!el) return;
  
  // Check if any orderRows exist
  var hasRows = LINES.some(function(l){ return (orderRows[l.id]||[]).length > 0; });
  if(!hasRows){
    el.innerHTML = '<div style="background:var(--card);border:1px solid var(--border);border-radius:12px;padding:16px;text-align:center;color:var(--dim);font-size:14px">יום שישי — אין תוכנית עבודה<br><span style="font-size:12px">שורות אדומות מוצגות בלבד</span></div>';
    return;
  }
  
  // Show red rows summary per line
  var html = '<div style="background:var(--red-bg);border:1px solid rgba(220,38,38,.2);border-radius:12px;padding:12px 14px;margin-bottom:10px">';
  html += '<div style="font-size:13px;font-weight:700;color:var(--red);margin-bottom:8px">יום שישי — שורות אדומות לטיפול</div>';
  html += '<div style="display:grid;grid-template-columns:repeat(5,1fr);gap:6px">';
  LINES.forEach(function(l){
    var rows = orderRows[l.id] || [];
    var redOpen = rows.filter(function(r){ return r.isRed && r.status !== 'done'; }).length;
    var redDone = rows.filter(function(r){ return r.isRed && r.status === 'done'; }).length;
    var col = redOpen > 0 ? 'var(--red)' : 'var(--green)';
    html += '<div style="background:var(--card);border-radius:8px;padding:8px;text-align:center;cursor:pointer" onclick="openLineDetail(\'' + l.id + '\')">';
    html += '<div style="font-size:11px;font-weight:700;margin-bottom:4px">' + l.name + '</div>';
    html += '<div style="font-size:20px;font-weight:900;font-family:var(--mono);color:' + col + '">' + redOpen + '</div>';
    html += '<div style="font-size:9px;color:var(--dim)">פתוחות</div>';
    if(redDone) html += '<div style="font-size:10px;color:var(--green)">&#10003; ' + redDone + ' סגורו</div>';
    html += '</div>';
  });
  html += '</div></div>';
  el.innerHTML = html;
...
loadState();
loadOrderRows();
setTimeout(function(){ loadSkuCatalog(); }, 100);

// Shift type
var _savedShift = store.getItem('ops_shift_type');
if(_savedShift){ currentShiftType = _savedShift; }
else {
  var _d = new Date().getDay();
  currentShiftType = _d===5?'short':(_d===0||_d===2||_d===4)?'regular':'long';
  store.setItem('ops_shift_type', currentShiftType);
}
...
// Pending reminders
setTimeout(function(){
  try{ checkMaterialAlerts(); checkRedRowsAge(); } catch(e){}
}, 3000);

function updatePalletInstr(palletId, val){
  var p=pallets.find(function(x){return x.id===palletId;});
  if(p) p.instructions=val;
  debouncePalletSave();
}

function toggleRedRowReason(lineId, rowIdx){
  var box=document.getElementById('rr-reason-'+lineId+'-'+rowIdx);
  if(box) box.style.display=box.style.display==='none'||!box.style.display?'block':'none';
}

function saveRedRowReason(lineId, rowIdx){
  var rows=orderRows[lineId]||[];
  if(!rows[rowIdx]) return;
  var reason=(document.getElementById('rr-sel-'+lineId+'-'+rowIdx)||{}).value||'';
  var missingsku=(document.getElementById('rr-sku-'+lineId+'-'+rowIdx)||{}).value||'';
  var note=(document.getElementById('rr-note-'+lineId+'-'+rowIdx)||{}).value||'';
  rows[rowIdx].delayReason=reason;
  rows[rowIdx].missingsku=missingsku;
  rows[rowIdx].delaynote=note;
  orderRows[lineId]=rows;
  saveOrderRows();
  renderLineRows();
  showT('סיבה נשמרה');
...
function renderDelayReasonsMgr(){
  var el=document.getElementById('delay-reasons-mgr');
  if(!el) return;
  var html='<div style="display:flex;flex-wrap:wrap;gap:6px;margin-bottom:10px">';
  delayReasons.forEach(function(r,i){
    html+='<span style="font-size:12px;padding:4px 12px;background:var(--red-bg);color:var(--red);border:1px solid rgba(220,38,38,.2);border-radius:16px;display:flex;align-items:center;gap:6px">';
    html+=r;
    html+='<button data-idx="'+i+'" onclick="removeDelayReason(parseInt(this.dataset.idx))" style="background:none;border:none;color:var(--red);cursor:pointer;font-size:12px;padding:0;line-height:1">✕</button>';
    html+='</span>';
  });
  html+='</div>';
  html+='<div style="display:flex;gap:8px">';
  html+='<input type="text" id="new-delay-reason" placeholder="הוסף סיבה..." autocomplete="off" style="flex:1;background:var(--surface);border:1px solid var(--border);border-radius:7px;padding:7px 10px;color:var(--text);font-size:13px">';
  html+='<button onclick="addDelayReason()" style="padding:7px 14px;background:var(--blue);border:none;border-radius:7px;color:#fff;font-size:13px;font-weight:700;cursor:pointer">הוסף</button>';
  html+='</div>';
  el.innerHTML=html;
}

function addDelayReason(){
  var inp=document.getElementById('new-delay-reason');
  if(!inp||!inp.value.trim()) return;
  delayReasons.push(inp.value.trim());
  inp.value='';
  saveState();renderDelayReasonsMgr();showT('נוסף!');
}

function removeDelayReason(idx){
  delayReasons.splice(idx,1);
  saveState();renderDelayReasonsMgr();
}
...
function renderPalletHistory(){
  ...
  var html='<div style="font-size:12px;font-weight:700;color:var(--dim);margin-bottom:8px">'+hist.length+' משטחים בהיסטוריה</div>';
  Object.keys(groups).forEach(function(dk){
    var dayPallets=groups[dk];
    var closed=dayPallets.filter(function(p){return p.closed;}).length;
    html+='<div style="font-size:12px;font-weight:700;color:var(--dim);padding:6px 0;border-bottom:1px solid var(--border);margin-bottom:6px;display:flex;justify-content:space-between">';
    html+='<span>'+dk+'</span>';
    html+='<span style="color:var(--green)">נסגרו: '+closed+' / '+dayPallets.length+'</span>';
    html+='</div>';
    dayPallets.forEach(function(p){
      var shipCol=p.shipType==='air'?'var(--blue)':'var(--green)';
      var shipLabel=p.shipType==='air'?'✈ אוויר':'⛵ ים';
      html+='<div style="padding:8px 10px;background:var(--card);border:1px solid var(--border);border-radius:8px;margin-bottom:5px">';
      html+='<div style="display:flex;justify-content:space-between;align-items:center">';
      html+='<div>';
      html+='<span style="font-size:13px;font-weight:700;color:var(--blue)">'+(p.orderId||'-')+'</span>';
      if(p.customer) html+=' <span style="font-size:11px;color:var(--dim)">'+p.customer+'</span>';
      html+='</div>';
      html+='<div style="display:flex;gap:6px;align-items:center">';
      html+='<span style="font-size:10px;color:'+shipCol+'">'+shipLabel+'</span>';
      html+='<span style="font-size:11px;color:var(--dim)">'+p.pkNum+'</span>';
      if(p.closedAt) html+='<span style="font-size:10px;color:var(--dim)">נסגר: '+p.closedAt+'</span>';
      html+='</div></div>';
      var calc=calcPalletHeight(p);
      html+='<div style="font-size:11px;color:var(--dim);margin-top:2px">'+calc.tot+' קרטונים | '+calc.totalCm+' ס"מ'+(p.instructions?' | '+p.instructions:'')+'</div>';
      html+='</div>';
    });
    html+='<div style="margin-bottom:10px"></div>';
  });
