...
function syncFromServer(){
  ...
  sbFetch('ops_data?select=key,value').then(function(rows){
    ...
    saveState();
    // Don't re-render dash if user has open line (would lose focus)
    var _ae=document.activeElement;
    var _userTyping=_ae&&(_ae.tagName==='INPUT'||_ae.tagName==='TEXTAREA')&&_ae.id==='dash-row-search';
    var _searchEl=document.getElementById('dash-row-search');
    var _searchFocused=document.activeElement===_searchEl;
    if(!_searchFocused){renderDash();}
...
function renderDash(){
  ...
  if(openLineId){
    ...
    if(openCalc&&openLine){
      ...
      filtered2.forEach(function(r){
        ...
        inlineRows+='<div data-search="'+(r.sku||'')+' '+(r.order||'')+' '+(r.customer||'')+' '+(r.skuB||'')+' '+(r.expiry||'')+'" style="background:'+bg+';border:'+border+';border-radius:12px;padding:12px 14px;margin-bottom:8px">';
  ...
  // Don't rebuild inline rows if user is actively typing in search
  var _searchActive=document.activeElement&&document.activeElement.id==='dash-row-search';
  if(!_searchActive){
