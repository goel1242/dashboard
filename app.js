...
function renderLineRows(){
  ...
  filtered.forEach(function(r){
    ...
    if(r.customer) html+='<div style="font-size:13px;font-weight:600;color:var(--dim);margin-bottom:3px">'+r.customer+'</div>';
    // Order + line
    if(r.order) html+='<div style="display:flex;align-items:baseline;gap:6px;margin-bottom:6px"><span style="font-size:16px;font-weight:700;color:'+orderCol+'">'+r.order+'</span><span style="font-size:12px;color:var(--dim)">/ שורה '+(r.orderLine||'')+'</span></div>';
    // Desc
    if(r.desc) html+='<div style="font-size:16px;font-weight:700;color:'+textCol+';margin-bottom:8px;line-height:1.3">'+r.desc+'</div>';
    // SKU badge
    if(r.sku){
      var skuBg=isRed?'rgba(252,235,235,0.8)':'var(--surface)';
      html+='<div style="background:'+skuBg+';border-radius:7px;padding:5px 10px;display:inline-block;margin-bottom:5px">';
      html+='<div style="font-size:10px;color:'+(isRed?'#E24B4A':'var(--dim)')+';margin-bottom:1px">מק"ט</div>';
      html+='<div style="font-size:15px;font-weight:700;font-family:var(--mono);color:'+textCol+'">'+r.sku+'</div>';
      html+='</div>';
    }
    // skuB badge
    if(showSkuB&&r.skuB){
      html+='<div style="background:var(--blue-bg);border-radius:7px;padding:5px 10px;display:inline-block;margin-right:5px">';
    ...
    if(isRed&&!isDone){
      ...
      ['חסר חומר','בעיית איכות','ממתין להנדסה','לקוח עצור','אחר'].forEach(function(opt){
...
function loadSkuCatalog(){
  ...
  var p1=typeof SKU_CATALOG_1!=='undefined'?SKU_CATALOG_1:[];
  var p2=typeof SKU_CATALOG_2!=='undefined'?SKU_CATALOG_2:[];
  ...
  if(skuCatalog.length){
    ...
    setTimeout(function(){
      var p1b=typeof SKU_CATALOG_1!=='undefined'?SKU_CATALOG_1:[];
      var p2b=typeof SKU_CATALOG_2!=='undefined'?SKU_CATALOG_2:[];
