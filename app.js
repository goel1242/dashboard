...
function mergeOrderRows(local, remote){
  // For each line, merge rows by order+orderLine key
  // Keep higher done value, preserve local additions array
  var result={};
  ...
  allLines.forEach(function(lid){
    ...
    var merged=remoteRows.map(function(rr,i){
      ...
      var lr=localMap[key];
      if(!lr) return rr;
      // Keep higher done, preserve local additions
      var localDone=lr.done||0;
      var remoteDone=rr.done||0;
      if(localDone>=remoteDone){
        return lr; // local is newer/higher
      } else {
        // Remote is higher but keep local additions log
        var merged2=Object.assign({},rr);
        if(lr.additions&&lr.additions.length) merged2.additions=lr.additions;
        return merged2;
      }
    });
