...
function cleanupCompletedWorkOrders(){
  ...
  Object.keys(workOrderData).forEach(function(peka){
    ...
    var row=rows.filter(function(r){return (r.order||'')===wo.matchedOrder&&(r.orderLine||'')===wo.matchedOrderLine;})[0];
...
function syncFromServer(){
  ...
  sbFetch('ops_data?select=key,value').then(function(rows){
    ...
    if(map.work_orders) try{workOrderData=JSON.parse(map.work_orders);}catch(e){}
    if(map.order_rows){try{
      var _or=JSON.parse(map.order_rows);
      var sr2=(_or&&_or._ts&&_or.data)?_or.data:_or;
...
function renderPallets(){
  ...
  pallets.forEach(function(p){
    ...
    if(!isOpen){
      ...
      if(p.customer) html+='<span style="font-size:11px;color:var(--dim);white-space:nowrap;overflow:hidden;text-overflow:ellipsis">'+p.customer+'</span>';
...
function matchWorkOrdersToRows(parsed){
  ...
  Object.keys(parsed).forEach(function(peka){
    ...
    if(found){
      ...
      unmatched.push(peka+(wo.customer?' ('+wo.customer+')':''));
    }
  });
...
function getWorkOrderForRow(lineId,order,orderLine){
  ...
  Object.keys(workOrderData).forEach(function(peka){
    if(found) return;
    var wo=workOrderData[peka];
    if(wo.lineId===lineId&&wo.matchedOrder===order&&wo.matchedOrderLine===orderLine) found={peka:peka,wo:wo};
  });
  return found;
}
function openWorkOrderDetail(peka){
  ...
  if(subEl) subEl.textContent=(wo.customer||'')+(wo.qty?' \u00b7 '+wo.qty+' \u05d9\u05d7\'':'');
    ...
    openRows.forEach(function(r){
      ...
      html+='<div style="background:var(--card);border:1px solid rgba(220,38,38,.3);border-radius:10px;padding:10px 12px;margin-bottom:8px">'+
        '<div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:8px">'+
          '<div><div style="font-size:12.5px;font-weight:700;font-family:var(--mono)">'+escapeHtml(r.sku||'')+'</div><div style="font-size:10.5px;color:var(--dim)">'+escapeHtml(r.desc||'')+(r.order?' \u00b7 \u05d4\u05d6\u05de\u05e0\u05d4 '+escapeHtml(r.order):'')+(r.customer?' \u00b7 '+escapeHtml(r.customer):'')+'</div></div>'+
...
function renderLineRows(){
  ...
  filtered.forEach(function(r){
    ...
    var _ds=escapeHtml((r.sku||'')+' '+(r.order||'')+' '+(r.customer||'')+' '+(r.skuB||'')+' '+(r.expiry||''));
...
function buildDashInlineRows(openLineId){
  ...
  filtered2.forEach(function(r){
    ...
    html+='<div data-search="'+(r.sku||'')+' '+(r.order||'')+' '+(r.customer||'')+' '+(r.skuB||'')+' '+(r.expiry||'')+'" data-order="'+(r.order||'')+'" data-sku="'+(r.sku||'')+'" data-skub="'+(r.skuB||'')+'" data-expiry="'+(r.expiry||'')+'" style="background:'+bg+';border:'+border+';border-radius:12px;padding:12px 14px;margin-bottom:8px">';
    html+='<div style="display:flex;justify-content:space-between;align-items:flex-start;gap:10px">';
    // Left side
    html+='<div style="flex:1;min-width:0">';
    if(r.customer) html+='<div style="font-size:15px;font-weight:800;color:#0F172A;margin-bottom:3px">'+r.customer+'</div>';
    if(r.order) html+='<div style="font-size:13px;font-weight:600;color:'+orderCol+'">'+r.order+'<span style="font-size:11px;font-weight:400;color:#64748B"> / שורה '+(r.orderLine||'')+'</span></div>';
