...
function renderLineRows(){
  ...
  html+='</div>';
  // Search
  html+='<input type="search" id="lt-search" inputmode="text" autocomplete="off" autocorrect="off" autocapitalize="off" spellcheck="false" placeholder="חפש הזמנה / מק-ט / לקוח..." oninput="renderLineRows()" value="'+search+'" style="width:100%;background:var(--surface);border:1px solid var(--border);border-radius:8px;padding:8px 12px;color:var(--text);font-size:13px;margin-bottom:8px">';
  ...
  filtered.forEach(function(r){
    ...
    if(isRed&&!isDone){
      ...
      html+='</select>';
      html+='<input type="text" id="rr-sku-'+currentLine+'-'+realIdx+'" placeholder="מק"ט חסר..." autocomplete="off" autocapitalize="off" value="'+(r.missingsku||'')+'" style="width:100%;background:var(--surface);border:1px solid var(--border);border-radius:7px;padding:6px 10px;color:var(--text);font-size:13px;font-family:var(--mono);margin-bottom:6px">';
...
function loadSkuCatalog(){
  ...
  if(skuCatalog.length){
    ...
    setTimeout(function(){
      ...
      var p2=typeof SKU_CATALOG_2!=='undefined'?SKU_CATALOG_2:[];
      skuCatalog=p1.concat(p2);
      if(skuCatalog.length){loadCatalogEdits();console.log('SKU catalog loaded (retry):',skuCatalog.length);}
    }, 800);
  }
}
