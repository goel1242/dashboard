...
function clearDay(){
  // Archive only CLOSED pallets to history, keep open ones
  if(pallets.length){
    try{
      var ph=JSON.parse(store.getItem('ops_pallet_history')||'[]');
      pallets.filter(function(p){return p.closed;}).forEach(function(p){
  ...
  // Keep only OPEN pallets (not closed ones)
  pallets=pallets.filter(function(p){return !p.closed;});
  ...
  sbFetch('ops_data?select=key,value').then(function(rows){
    ...
    if(map.order_rows){
      try{
        ...
        if(syncCount>0){orderRows=syncedRows;recalcRedRows();}
      }catch(e){}
    }
    if(map.workers)   DEFAULT_WORKERS=JSON.parse(map.workers);
    saveState();
    // Don't re-render dash if user typed in search recently (3 second window)
    var _recentSearch=window._lastSearchTime&&(Date.now()-window._lastSearchTime<3000);
    if(!_recentSearch){
      if(document.getElementById('page-dash').classList.contains('active')){renderDash();}
      else{window._dashNeedsRender=true;}
    }
...
function goPage(name){
  ...
  if(name==='dash'){if(!document.getElementById('lines-list').innerHTML.trim()||window._dashNeedsRender){renderDash();window._dashNeedsRender=false;}}
  if(name==='stops'){if(!window._stopsRendered||window._stopsNeedsRender){renderStops();window._stopsRendered=true;window._stopsNeedsRender=false;}}
  if(name==='pack'){if(!window._packRendered){buildRequestForms();renderPallets();renderPackRequests();window._packRendered=true;}else{renderPackRequests();}}
...
function saveMorning(){
  ...
  saveState();syncToServer();buildMorningForms();renderDash();window._dashNeedsRender=false;
...
function renderDash(){
  ...
  var cardsHtml='<div style="display:grid;grid-template-columns:repeat(5,minmax(58px,1fr));gap:5px;margin-bottom:8px;overflow:hidden">';
  cardsHtml+=calcs.map(function(c){
    ...
    // Fallback: use manual redRows entry when no order rows loaded
    if(!rows.length){
      var _rr=redRows[c.id]||{};var _rc=redClosed[c.id]||{};
      redOpen=Math.max(0,(_rr.rows||0)-(_rc.rows||0));
    }
    ...
    return '<button data-lineid="'+c.id+'" onclick="dashToggleLine(this.dataset.lineid)" style="background:'+cardBg+';border:'+cardBorder+';border-radius:12px;padding:10px 8px;cursor:pointer;text-align:center;width:100%;font-family:Heebo,sans-serif;-webkit-tap-highlight-color:transparent">'+
  ...
  if(openLineId){
    ...
    if(openCalc&&openLine){
      ...
      inlineRows+='<div id="dash-inline-rows-'+openLineId+'" style="margin-top:0">';
      ...
      inlineRows+='<div id="dash-filter-btns" style="display:flex;gap:5px;margin-bottom:6px;flex-wrap:nowrap;overflow-x:auto;-webkit-overflow-scrolling:touch">';
      _filters.forEach(function(f){
        var active=_sf===f[0];
        inlineRows+='<button data-f="'+f[0]+'" onclick="setDashFilter(this.dataset.f)" style="font-size:10px;padding:5px 10px;border-radius:16px;border:1.5px solid '+(active?'#2563EB':'var(--border)')+';background:'+(active?'#2563EB':'var(--surface)')+';color:'+(active?'#fff':'var(--dim)')+';white-space:nowrap;flex-shrink:0;cursor:pointer;font-family:var(--font)">'+f[1]+'</button>';
      });
      inlineRows+='</div>';
      var _ph={'all':'חפש הזמנה / מק-ט / מק-ט בן / פק-ע...','order':'חפש לפי הזמנה...','sku':'חפש לפי מק"ט...','skuB':'חפש לפי מק"ט בן...','expiry':'חפש לפי פק"ע...'};
      inlineRows+='<input type="search" id="dash-row-search" autocomplete="off" autocorrect="off" autocapitalize="off" spellcheck="false" placeholder="'+(_ph[_sf]||_ph.all)+'" value="'+(window._dashSearch||'')+'" style="width:100%;background:var(--surface);border:1px solid var(--border);border-radius:8px;padding:7px 12px;color:var(--text);font-size:13px;margin-bottom:8px">';
      ...
      filtered2.forEach(function(r){
        ...
        inlineRows+='<div data-search="'+(r.sku||'')+' '+(r.order||'')+' '+(r.customer||'')+' '+(r.skuB||'')+' '+(r.expiry||'')+'" data-order="'+(r.order||'')+'" data-sku="'+(r.sku||'')+'" data-skub="'+(r.skuB||'')+'" data-expiry="'+(r.expiry||'')+'" style="background:'+bg+';border:'+border+';border-radius:12px;padding:12px 14px;margin-bottom:8px">';
  ...
  var _recentSearch3=window._lastSearchTime&&(Date.now()-window._lastSearchTime<3000);
  if(!_recentSearch3){
    list.innerHTML=cardsHtml;
  }
  // Add inline rows in a separate sibling div to avoid grid clipping
  var inlineDiv=document.getElementById('dash-inline-section');
  if(!inlineDiv){
    inlineDiv=document.createElement('div');
    inlineDiv.id='dash-inline-section';
    list.parentNode.insertBefore(inlineDiv,list.nextSibling);
  }
  // Don't rebuild inline rows if user searched recently
  var _recentSearch2=window._lastSearchTime&&(Date.now()-window._lastSearchTime<3000);
  if(!_recentSearch2){
    inlineDiv.innerHTML=inlineRows;
    // Re-apply filter if there's a search term
    if(window._dashSearch) setTimeout(function(){filterDashRows(window._dashSearch);},10);
  }
  ...
  if(searchInp){
    ...
    if(savedVal) setTimeout(function(){filterDashRows(savedVal);},10);
  }
}
...
function filterDashRows(term){
  ...
  Array.prototype.forEach.call(cards,function(card){
    ...
    var order=(card.getAttribute('data-order')||'').toLowerCase();
    var sku=(card.getAttribute('data-sku')||'').toLowerCase();
    var skuB=(card.getAttribute('data-skub')||'').toLowerCase();
    var expiry=(card.getAttribute('data-expiry')||'').toLowerCase();
    var match=false;
...
function dashToggleLine(lineId){
  if(window._dashOpenLine===lineId){
    ...
    window._dashSearch='';
    currentLine=lineId;
    // Clear search display when opening new line
    filterDashRows('');
  }
  renderDash();
  if(window._dashOpenLine){
    setTimeout(function(){
      // Scroll to inline rows section
      var rowsEl=document.getElementById('dash-inline-section');
      if(rowsEl&&rowsEl.innerHTML.trim()){
        rowsEl.scrollIntoView({behavior:'smooth',block:'start'});
      }
    },100);
  }
}
...
function openStop(){
  ...
  saveState();syncToServer();updateStopBdg();window._stopsNeedsRender=true;if(document.getElementById('page-stops').classList.contains('active')){renderStops();window._stopsNeedsRender=false;}
...
function saveOrderRows(){
  ...
  saveState();
  // Sync without re-rendering - let the interval handle it
  sbFetch('ops_data',{method:'POST',prefer:'resolution=merge-duplicates',body:JSON.stringify([
    {key:'order_rows',value:JSON.stringify(orderRows),updated_at:new Date().toISOString()},
    {key:'reports',value:JSON.stringify(reports),updated_at:new Date().toISOString()},
    {key:'targets',value:JSON.stringify(targets),updated_at:new Date().toISOString()}
  ])}).catch(function(){});
}
...
function npSave(){
  ...
  saveState();syncToServer();runAlerts();renderDash();window._dashNeedsRender=false;
...
function renderDashRows(){
  ...
  // Find the rows container inside the inline section
  var container=document.getElementById('dash-inline-rows-'+openLineId);
  if(!container) return;
  ...
  filtered2.forEach(function(r){
    ...
    if(r.sku) html+='<div style="background:'+(isDone||isRed||isPartial?'#fff':'#F8FAFC')+';border:1px solid '+skuBorder+';border-radius:6px;padding:3px 8px"><div style="font-size:8px;color:'+skuLabel+';font-weight:700">מק"ט</div><div style="font-size:12px;font-weight:700;font-family:var(--mono);color:#1E293B">'+r.sku+'</div></div>';
    if(showSkuB2&&r.skuB) html+='<div style="background:#EFF6FF;border:1px solid #BFDBFE;border-radius:6px;padding:3px 8px"><div style="font-size:8px;color:#1D4ED8;font-weight:700">מק"ט בן</div><div style="font-size:12px;font-weight:700;font-family:var(--mono);color:#1E40AF">'+r.skuB+'</div></div>';
    html+='</div></div>';
    ...
    if(isDone){
      ...
      html+='<button data-lid="'+openLineId+'" data-idx="'+realIdx+'" onclick="undoRowMark(this.dataset.lid,parseInt(this.dataset.idx));renderDash()" style="font-size:10px;padding:2px 10px;background:#fff;border:1px solid rgba(34,197,94,.3);border-radius:5px;color:#16A34A;cursor:pointer">בטל</button>';
    } else if(isPartial){
      ...
      html+='<button data-lid="'+openLineId+'" data-idx="'+realIdx+'" onclick="undoRowMark(this.dataset.lid,parseInt(this.dataset.idx));renderDash()" style="font-size:10px;padding:2px 10px;background:#fff;border:1px solid rgba(234,88,12,.3);border-radius:5px;color:#EA580C;cursor:pointer">בטל</button>';
    } else {
      html+='<div><div style="font-size:20px;font-weight:800;font-family:var(--mono);color:#0F172A">'+r.qty+'</div><div style="font-size:9px;color:#64748B">יח לאריזה</div></div>';
      html+='<div style="display:flex;gap:4px">';
      html+='<button data-pidx="'+realIdx+'" onclick="var p=document.getElementById(\"dr-pinput-\"+this.dataset.pidx);if(p)p.style.display=p.style.display===\"flex\"?\"none\":\"flex\"" style="width:32px;height:32px;background:#F5C4B3;color:#993C1D;border:0.5px solid #F0997B;border-radius:7px;font-size:14px;cursor:pointer">~</button>';
      html+='<button data-lid="'+openLineId+'" data-idx="'+realIdx+'" onclick="markRowDone(this.dataset.lid,parseInt(this.dataset.idx));renderDash()" style="width:32px;height:32px;background:#C0DD97;color:#3B6D11;border:0.5px solid #97C459;border-radius:7px;font-size:15px;cursor:pointer">&#10003;</button>';
      html+='</div>';
    }
    html+='</div></div>';
    if(!isDone){
      if(r.rowReason){
        html+='<div style="background:#F0FDF4;border:1px solid rgba(5,150,105,.25);border-radius:8px;padding:6px 10px;margin-top:7px;display:flex;justify-content:space-between"><div><div style="font-size:9px;color:#64748B">סיבת אי ביצוע</div><div style="font-size:12px;font-weight:700;color:#059669">'+r.rowReason+'</div>'+(r.rowNote?'<div style="font-size:11px;color:#334155">'+r.rowNote+'</div>':'')+'</div><button data-lid="'+openLineId+'" data-idx="'+realIdx+'" onclick="toggleDashRowReason(this.dataset.lid,parseInt(this.dataset.idx))" style="font-size:9px;padding:2px 7px;background:#fff;border:1px solid #E2E8F0;border-radius:5px;color:#64748B;cursor:pointer">ערוך</button></div>';
      } else {
      ...
      html+='<div id="dr-reason-'+openLineId+'-'+realIdx+'" style="display:none;background:#FFFBEB;border:1px solid rgba(234,179,8,.3);border-radius:8px;padding:9px 11px;margin-top:6px">';
      html+='<select id="dr-sel-'+openLineId+'-'+realIdx+'" style="width:100%;background:#fff;border:1px solid rgba(234,179,8,.4);border-radius:7px;padding:6px 9px;color:#1E293B;font-size:13px;font-family:Heebo,sans-serif;margin-bottom:6px">';
      ['בחר סיבה...','מחסן','ליקוט','סט-אפ','הנדסה','פיתוח','ייצור','איכות','IT','תכנון','שיווק','אחר'].forEach(function(opt){html+='<option>'+opt+'</option>';});
      html+='</select><textarea id="dr-note-'+openLineId+'-'+realIdx+'" rows="2" placeholder="פרט..." style="width:100%;background:#fff;border:1px solid rgba(234,179,8,.4);border-radius:7px;padding:6px 9px;color:#1E293B;font-size:13px;font-family:Heebo,sans-serif;resize:none;margin-bottom:6px">'+(r.rowNote||'')+'</textarea>';
      html+='<div style="display:flex;gap:6px"><button data-lid="'+openLineId+'" data-idx="'+realIdx+'" onclick="saveDashRowReason(this.dataset.lid,parseInt(this.dataset.idx))" style="flex:1;padding:7px;background:var(--green);border:none;border-radius:7px;color:#fff;font-size:13px;font-weight:700;cursor:pointer">שמור</button><button data-lid="'+openLineId+'" data-idx="'+realIdx+'" onclick="toggleDashRowReason(this.dataset.lid,parseInt(this.dataset.idx))" style="padding:7px 12px;background:#fff;border:1px solid #E2E8F0;border-radius:7px;color:#64748B;font-size:13px;cursor:pointer">ביטול</button></div></div>';
