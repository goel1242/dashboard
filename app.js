...
function buildDashInlineRows(openLineId){
  ...
  filtered2.forEach(function(r){
    ...
    if(!isDone){
      ...
      html+='<div id="dr-pinput-'+realIdx+'" style="display:none;align-items:center;gap:8px;margin-top:9px;background:#F8FAFC;border:1px solid #E2E8F0;border-radius:8px;padding:7px 10px"><span style="font-size:11px;color:#64748B;white-space:nowrap">+ הוסף:</span><input type="number" inputmode="numeric" placeholder="כמה?" id="dr-partial-'+realIdx+'" min="1" style="flex:1;text-align:center;font-size:20px;font-weight:800;font-family:var(--mono);background:transparent;border:none;outline:none;color:#334155;min-width:0"><button data-lid="'+openLineId+'" data-pidx="'+realIdx+'" onclick="addRowQtyFromInput(this.dataset.lid,this.dataset.pidx)" style="padding:6px 14px;background:#334155;border:none;border-radius:7px;color:#fff;font-size:13px;font-weight:700;cursor:pointer;font-family:Heebo,sans-serif">הוסף</button></div>';
    }
...
function addRowQty(lineId, rowIdx, amount){
  var rows=orderRows[lineId]||[];
  if(!rows[rowIdx]) return;
  var row=rows[rowIdx];
  if(!row.additions) row.additions=[];
  var amt=parseInt(amount)||0;
  if(amt<=0){showT('הזן כמות חיובית','y');return;}
  var remaining=row.qty-(row.done||0);
  if(amt>remaining) amt=remaining;
  row.additions.push({qty:amt,time:nowTime()});
  row.done=(row.done||0)+amt;
  row.status=row.done>=row.qty?'done':'partial';
