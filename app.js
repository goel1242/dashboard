...
function renderRedRowsManager(){
  ...
  if(!all.length){
    el.innerHTML='<div class="empty">אין שורות אדומות פתוחות</div>';
    return;
  }
  // Group by line
  var byLine={};
...
function renderMatHistory(){
  ...
  if(!materialRequests.length){
    el.innerHTML='<div class="empty">אין היסטוריה</div>';
    return;
  }
  var sorted=materialRequests.slice().sort(function(a,b){return (b.createdTs||0)-(a.createdTs||0);});
  var html='';
  sorted.forEach(function(req){
    ...
    var dur=req.endTs&&req.createdTs?Math.round((req.endTs-req.createdTs)/60000)+'min':'פתוח';
    var d=new Date(req.createdTs||Date.now());
    var dateStr=d.toLocaleDateString('he-IL')+' '+pad(d.getHours())+':'+pad(d.getMinutes());
