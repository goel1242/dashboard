...
function renderLineRows(){
  ...
  var pallets2=sorted;
  // NEW: sort newest pallets first
  pallets.sort(function(a,b){
    var da=new Date(a.createdDateObj||a.id||0);
    var db=new Date(b.createdDateObj||b.id||0);
    return db-da;
  });
  ...
  filtered.forEach(function(r){
    ...
    if(r.desc) html+='<div style="font-size:14px;font-weight:600;color:#334155;margin-bottom:6px">'+r.desc+'</div>';
    if(r.sku){
    ...
    if(showSkuB&&r.skuB){
      html+='<div style="background:#EFF6FF;border:1px solid #BFDBFE;border-radius:7px;padding:4px 8px;display:inline-block">';
      html+='<div style="font-size:10px;color:var(--blue)">\u05de\u05e7"\u05d8 \u05d1\u05df</div>';
      html+='<div style="font-size:14px;font-weight:700;font-family:var(--mono);color:var(--blue)">'+r.skuB+'</div>';
      html+='</div>';
    }
    html+='</div>';
    // Right
    ...
    if(showExpiry&&r.expiry){
      ...
      html+='<div style="font-size:9px;color:'+expiryLbl+';font-weight:700">\u05e4\u05e7"\u05e2</div>';
    ...
    if(isDone||isPartial){
      ...
      if(r.completedAt){
        html+='<div style="display:flex;align-items:center;gap:4px;background:#fff;border:1px solid '+(isDone?'rgba(34,197,94,.3)':'rgba(234,88,12,.3)')+';border-radius:6px;padding:2px 8px">';
        html+='<span style="font-size:10px;color:'+dimCol+'">'+(isDone?'\u05d4\u05d5\u05e9\u05dc\u05dd':'\u05e2\u05d5\u05d3\u05db\u05df')+' \u05d1-</span>';
        html+='<span style="font-size:12px;font-weight:700;font-family:var(--mono);color:'+doneCol+'">'+r.completedAt+'</span>';
        html+='</div>';
      }
...
var hourlyLog={};
function loadHourlyLog(){try{hourlyLog=JSON.parse(store.getItem('ops_hourly_log')||'{}');}catch(e){hourlyLog={};}}
...
function checkRedRowsAge(){
  ...
  LINES.forEach(function(l){
    ...
    rows.forEach(function(r){
      ...
      if(diffDays>=2) pushAlert('r','\u05e9\u05d5\u05e8\u05d4 \u05d0\u05d3\u05d5\u05de\u05d4 '+diffDays+' \u05d9\u05de\u05d9\u05dd: '+l.name+' - '+r.sku);
    });
  });
}
...
function skuLookup(sku){
  ...
  return skuCatalog.find(function(item){return item.sku.toUpperCase()===s;})||null;
}
function skuSearch(query,lineId){
  if(!query||query.length<2) return [];
  var q=query.toLowerCase();
  return skuCatalog.filter(function(item){
    var matchLine=!lineId||!item.lineId||item.lineId===lineId;
    return matchLine&&(item.sku.toLowerCase().includes(q)||item.desc.toLowerCase().includes(q));
  }).slice(0,8);
}
function renderCatalogSuggestions(inputId,lineId){
  ...
  box.innerHTML=results.map(function(r){
    return '<div onclick="selectCatalogItem(\''+inputId+'\',\''+r.sku.replace(/'/g,"\\'")+'\')" style="padding:8px 12px;cursor:pointer;border-bottom:1px solid var(--border);font-size:13px">'+
...
function selectCatalogItem(inputId,sku){
  var inp=document.getElementById(inputId);
  var box=document.getElementById(inputId+'-suggestions');
...
function renderCatalogStats(){
  ...
  var html='<div style="font-size:12px;font-weight:700;color:var(--dim);margin-bottom:8px">'+skuCatalog.length+' \u05de\u05e7"\u05d8\u05d9\u05dd</div>';
...
function calcLineETA(lineId){
  ...
  var totalSeconds=0,unknownQty=0;
  openRows.forEach(function(r){
    var remaining=(r.qty||0)-(r.done||0);
    if(remaining<=0) return;
    var cat=skuLookup(r.sku);
    if(cat&&cat.duration>0) totalSeconds+=cat.duration*remaining;
    else unknownQty+=remaining;
  });
  var totalHours=totalSeconds/3600;
  var adjustedHours=totalHours/Math.max(1,workers);
  return{minutes:Math.round(adjustedHours*60),hours:adjustedHours.toFixed(1),workers:workers,totalMinutes:Math.round(totalHours*60),unknownQty:unknownQty,openRows:openRows.length};
}
function renderLineETAs(){
  ...
  LINES.forEach(function(l){
    ...
    workerHtml+='<div style="text-align:center"><div style="font-size:11px;color:var(--dim);margin-bottom:3px">'+l.name+'</div>';
  ...
  LINES.forEach(function(l){
    ...
    lineCards+='<span style="font-size:12px;padding:3px 12px;background:'+(onTime?'var(--green-bg)':'var(--red-bg)')+';color:'+col+';border-radius:10px;font-weight:700">'+statusLabel+'</span>';
    lineCards+='</div>';
    lineCards+='<div style="display:grid;grid-template-columns:repeat(3,1fr);gap:6px;margin-bottom:8px">';
    lineCards+='<div style="text-align:center;background:var(--surface);border-radius:8px;padding:8px"><div style="font-size:22px;font-weight:900;font-family:var(--mono);color:var(--blue)">'+totalHours.toFixed(1)+'</div><div style="font-size:10px;color:var(--dim)">\u05e9\u05e2\u05d5\u05ea TT</div></div>';
    lineCards+='<div style="text-align:center;background:var(--surface);border-radius:8px;padding:8px"><div style="font-size:22px;font-weight:900;font-family:var(--mono);color:'+col+'">'+adjustedHours.toFixed(1)+'</div><div style="font-size:10px;color:var(--dim)">'+workers+' \u05e2\u05d5\u05d1\u05d3\u05d9\u05dd</div></div>';
    lineCards+='<div style="text-align:center;background:var(--surface);border-radius:8px;padding:8px"><div style="font-size:22px;font-weight:900;font-family:var(--mono);color:var(--faint)">'+shiftLeft.toFixed(1)+'</div><div style="font-size:10px;color:var(--dim)">\u05e9\u05e2\u05d5\u05ea \u05e9\u05e0\u05d5\u05ea\u05e8\u05d5</div></div>';
    lineCards+='</div>';
    if(!onTime) lineCards+='<div style="background:var(--red-bg);border:1px solid rgba(220,38,38,.2);border-radius:8px;padding:8px 12px;margin-bottom:8px;font-size:12px;color:var(--red)">\u05dc\u05e1\u05d9\u05d5\u05dd \u05d1\u05d6\u05de\u05df \u05e0\u05d3\u05e8\u05e9\u05d9\u05dd <strong>'+workersNeeded+' \u05e2\u05d5\u05d1\u05d3\u05d9\u05dd</strong> (\u05db\u05e8\u05d2\u05e2 '+workers+')</div>';
    if(unknownQty>0) lineCards+='<div style="font-size:10px;color:var(--yellow);margin-top:4px">'+unknownQty+' \u05d9\u05d7 \u05dc\u05dc\u05d0 TT</div>';
    lineCards+='</div>';
  });
  el.innerHTML=hasData?workerHtml+lineCards:workerHtml+'<div class="empty">\u05d8\u05e2\u05df \u05ea\u05d5\u05db\u05e0\u05d9\u05ea \u05e2\u05d1\u05d5\u05d3\u05d4 \u05ea\u05d7\u05d9\u05dc\u05d4</div>';
}
function renderRedRowsManager(){
  ...
  var all=[];
  LINES.forEach(function(l){
    (orderRows[l.id]||[]).forEach(function(r,idx){
      if(r.isRed&&r.status!=='done') all.push({line:l,row:r,idx:idx});
    });
  });
  if(!all.length){el.innerHTML='<div class="empty">\u05d0\u05d9\u05df \u05e9\u05d5\u05e8\u05d5\u05ea \u05d0\u05d3\u05d5\u05de\u05d5\u05ea \u05e4\u05ea\u05d5\u05d7\u05d5\u05ea</div>';return;}
  var byLine={};
  ...
  Object.values(byLine).forEach(function(group){
    ...
    html+='<div onclick="toggleRRGroup(\''+gid+'\')" style="font-size:13px;font-weight:700;color:var(--text);padding:6px 10px;background:var(--red-bg);border-radius:8px;border:1px solid rgba(220,38,38,.2);cursor:pointer;display:flex;justify-content:space-between">';
    html+='<span>'+group.line.name+' \u2014 '+group.rows.length+' \u05e9\u05d5\u05e8\u05d5\u05ea</span><span id="arr-'+gid+'">\u25B6</span></div>';
    ...
    group.rows.forEach(function(item){
      ...
      if(r.desc) html+='<div style="font-size:13px;color:var(--text)">'+r.desc+'</div>';
      ...
      html+='<div style="font-size:11px;color:var(--red)">'+r.qty+' \u05d9\u05d7'+(r.expiry?' | '+r.expiry+' \u26A0':'')+'</div>';
      html+='</div>';
    });
    html+='</div></div>';
  });
  ...
  LINES.forEach(function(l){
    (orderRows[l.id]||[]).forEach(function(r){
      if(r.isRed&&r.status!=='done'){
        all.push({'\u05e7\u05d5':l.name,'\u05d4\u05d6\u05de\u05e0\u05d4':r.order||'','\u05e9\u05d5\u05e8\u05d4':r.orderLine||'',
          '\u05de\u05e7"\u05d8':r.sku||'','\u05ea\u05d0\u05d5\u05e8':r.desc||'','\u05dc\u05e7\u05d5\u05d7':r.customer||'',
          '\u05db\u05de\u05d5\u05ea':r.qty||0,'\u05e4\u05e7"\u05e2':r.expiry||'','\u05de\u05e7"\u05d8 \u05d1\u05df':r.skuB||''});
      }
    });
  });
...
function renderFridayMode(){
  ...
  if(!hasRows){el.innerHTML='<div style="background:var(--card);border:1px solid var(--border);border-radius:12px;padding:16px;text-align:center;color:var(--dim);font-size:14px">\u05d9\u05d5\u05dd \u05e9\u05d9\u05e9\u05d9 \u2014 \u05d0\u05d9\u05df \u05ea\u05d5\u05db\u05e0\u05d9\u05ea \u05e2\u05d1\u05d5\u05d3\u05d4</div>';return;}
  ...
  LINES.forEach(function(l){
    ...
    html+='<div style="font-size:20px;font-weight:900;font-family:var(--mono);color:'+(redOpen>0?'var(--red)':'var(--green)')+'">'+redOpen+'</div>';
    html+='<div style="font-size:9px;color:var(--dim)">\u05e4\u05ea\u05d5\u05d7\u05d5\u05ea</div></div>';
  });
  html+='</div></div>';
  el.innerHTML=html;
}
...
// DELAY REASONS
// ============================================================
var delayReasons=['\u05d7\u05e1\u05e8 \u05d7\u05d5\u05de\u05e8','\u05d1\u05e2\u05d9\u05d9\u05ea \u05d0\u05d9\u05db\u05d5\u05ea','\u05de\u05de\u05ea\u05d9\u05df \u05d4\u05e0\u05d3\u05e1\u05d4','\u05ea\u05db\u05e0\u05d5\u05df','\u05e9\u05d9\u05d5\u05d5\u05e7','\u05e4\u05d9\u05ea\u05d5\u05d7','IT'];
...
function saveRedRowReason(lineId,rowIdx){
  ...
  orderRows[lineId]=rows;
  saveOrderRows();renderLineRows();showT('\u05e1\u05d9\u05d1\u05d4 \u05e0\u05e9\u05de\u05e8\u05d4');
}
function renderDelayReasonsMgr(){
  ...
  delayReasons.forEach(function(r,i){
    html+='<span style="font-size:12px;padding:4px 12px;background:var(--red-bg);color:var(--red);border:1px solid rgba(220,38,38,.2);border-radius:16px;display:flex;align-items:center;gap:6px">'+r;
    html+='<button data-idx="'+i+'" onclick="removeDelayReason(parseInt(this.dataset.idx))" style="background:none;border:none;color:var(--red);cursor:pointer;font-size:12px;padding:0">&#10005;</button>';
    html+='</span>';
  });
  html+='</div>';
  html+='<div style="display:flex;gap:8px">';
...
function renderCatalogManager(){
  ...
  if(edits.length){
    ...
    edits.forEach(function(item,i){
      ...
      html+='<div style="font-size:11px;color:var(--dim)">'+(item.desc||'')+' | '+(LINE_NAMES[item.lineId]||item.lineId||'')+' | '+item.duration+' \u05e9\u05e0</div></div>';
...
function addCatalogItem(){
  ...
  showT('\u05de\u05e7"\u05d8 \u05e0\u05e9\u05de\u05e8!');renderCatalogManager();renderCatalogStats();
}
...
// OTHER HELPERS
// ============================================================
function renderMorningTargets(){buildMorningForms();renderMgrSummary();}
function checkDayReset(){if(isNewDay()){var saved=store.getItem('ops_date');saveToHist(saved);clearDay();clearServerDay();}}
function toggleRRGroup(gid){
  var el=document.getElementById(gid);var arr=document.getElementById('arr-'+gid);
  if(!el) return;
...
function manualSaveHistory(){saveToHist(new Date().toDateString());showT('\u05d4\u05d9\u05d5\u05dd \u05e0\u05e9\u05de\u05e8 \u05dc\u05d4\u05d9\u05e1\u05d8\u05d5\u05e8\u05d9\u05d4!');}
...
renderAlerts();
updateMatBdg();
setTimeout(function(){try{checkMaterialAlerts();checkRedRowsAge();}catch(e){}},3000);
  ...
  if(linesEl){
    linesEl.innerHTML=LINES.map(function(l){
      ...
      return '<div style="background:var(--card);border:1px solid var(--border);border-radius:14px;padding:16px;text-align:center">'+
        '<span style="font-size:16px;font-weight:800">'+l.name+'</span><br>'+
        '<span style="font-size:48px;font-weight:900;font-family:var(--mono);color:'+dotCol+'">'+(calc.done!==null?calc.done:'-')+'</span>'+
        '<div style="font-size:12px;color:var(--dim)">מתוך '+calc.combined+'</div>'+
        '<div style="font-size:11px;color:var(--faint)">'+pp+'%</div></div>';
    }).join('');
  }
}
...
function renderDeleteOptions(){
  ...
  el.innerHTML=html||'<div class="empty" style="padding:16px">אין נתונים למחיקה</div>';
}
...
function buildHourlyMarkers(calc){return '';}
