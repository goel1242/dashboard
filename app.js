...
function renderLineRows(){
  ...
  // Sort: red rows first, then open, then partial, then done
  ...
  // Progress bar
  var html = '<div style="height:6px;background:var(--border);border-radius:3px;overflow:hidden;margin-bottom:12px">';
  ...
  sorted.forEach(function(r){
    ...
    html += '<div style="background:'+rowBg+';border:1px solid var(--border);border-radius:10px;padding:11px 14px;margin-bottom:7px">';

    // Header row: dot + order + customer
    html += '<div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:6px">';
    html += '<div style="display:flex;align-items:center;gap:8px">';
    html += '<span style="width:8px;height:8px;border-radius:50%;background:'+dotCol+';flex-shrink:0;display:inline-block;margin-top:2px"></span>';
    html += '<div>';
    if(r.order) html += '<div style="font-size:10px;color:var(--dim)">הזמנה '+r.order+(r.orderLine?' שורה '+r.orderLine:'')+'</div>';
    if(r.customer) html += '<div style="font-size:14px;font-weight:700;color:var(--text)">'+r.customer+'</div>';
    html += '</div></div>';

    // Done/qty or status badge
    if(isDone||isPartial){
      html += '<span style="font-size:15px;font-weight:800;font-family:var(--mono);color:'+(isDone?'var(--green)':'var(--yellow)')+'">'+r.done+'/'+r.qty+'</span>';
    } else {
      html += '<span style="font-size:11px;padding:2px 8px;border-radius:8px;background:'+(isRed?'var(--red-bg)':'var(--surface)')+';color:'+(isRed?'var(--red)':'var(--dim)')+';border:1px solid '+(isRed?'rgba(220,38,38,.2)':'var(--border)')+'">'+r.qty+' יח</span>';
    }
    html += '</div>';
    ...
    if(r.desc) html += '<span style="font-size:13px;color:var(--dim);font-weight:500">'+r.desc+'</span>';
    if(r.skuB) html += '<span style="font-size:10px;padding:1px 6px;background:var(--blue-bg);color:var(--blue);border-radius:4px">בן: '+r.skuB+'</span>';
    html += '</div>';

    // Details row: expiry + note
    if(r.expiry||r.isRed){
      html += '<div style="font-size:10px;color:'+(isRed?'var(--red)':'var(--dim)')+';margin-bottom:6px">פק"ע: '+(r.expiry||'-')+(isRed?' ⚠':'')+'</div>';
    }
    if(r.note) html += '<div style="font-size:10px;color:var(--yellow);margin-bottom:6px">'+r.note+'</div>';

    // Action buttons
    if(!isDone){
      html += '<div style="display:flex;gap:6px;margin-top:4px">';
      html += '<button data-lid="'+currentLine+'" data-idx="'+realIdx+'" onclick="markRowDone(this.dataset.lid,parseInt(this.dataset.idx));renderLineRows()" style="width:44px;height:38px;background:var(--green);border:none;border-radius:7px;color:#fff;font-size:16px;font-weight:700;cursor:pointer">&#10003;</button>';
      html += '<button data-lid="'+currentLine+'" data-idx="'+realIdx+'" onclick="toggleLtPartial(this)" style="width:44px;height:38px;background:var(--yellow-bg);border:1px solid rgba(217,119,6,.3);border-radius:7px;color:var(--yellow);font-size:13px;font-weight:700;cursor:pointer">~</button>';
      html += '</div>';
      // Partial input (hidden)
      html += '<div style="display:none;gap:6px;margin-top:6px">';
      html += '<input type="number" id="lt-partial-'+realIdx+'" inputmode="numeric" placeholder="כמה הושלם?" min="1" max="'+r.qty+'" style="flex:1;background:var(--surface);border:1px solid var(--border);border-radius:7px;padding:7px 10px;color:var(--text);font-size:14px;font-family:var(--mono);text-align:center">';
      html += '<button data-lid="'+currentLine+'" data-idx="'+realIdx+'" onclick="markLtPartial(this.dataset.lid,parseInt(this.dataset.idx))" style="padding:7px 14px;background:var(--yellow);border:none;border-radius:7px;color:#fff;font-size:13px;font-weight:700;cursor:pointer">שמור</button>';
      html += '</div>';
    } else {
      html += '<button data-lid="'+currentLine+'" data-idx="'+realIdx+'" onclick="undoRowMark(this.dataset.lid,parseInt(this.dataset.idx));renderLineRows()" style="font-size:10px;padding:3px 10px;background:var(--surface);border:1px solid var(--border);border-radius:5px;color:var(--faint);cursor:pointer;margin-top:4px">בטל</button>';
    }
    html += '</div>';
  });
