...
function syncToServer(){
  // Merge-safe push: for stoppages & pallets, pull whatever's on the server first and
  // merge it into local memory (by updatedAt, per record) BEFORE pushing - otherwise a
  // device with a stale local copy can blindly overwrite newer data another device already
  // synced, causing stoppages/pallets to "disappear" on other devices.
  sbFetch('ops_data?select=key,value&key=in.(stoppages,pallets)').then(function(rows){
...
function clearServerDay(){
  // Only clear order_rows on new day - other data cleared locally
  // Don't push empty values to Supabase to avoid wiping other devices
  var rows=[
    {key:'order_rows',value:'{}',updated_at:new Date().toISOString()}
  ];
