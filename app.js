...
function renderDash(){
  ...
  list.innerHTML=calcs.map(function(c){
    ...
    var dotCol=c.statusIdx===0||c.statusIdx===1?'var(--green)':c.statusIdx===2?'var(--yellow)':c.statusIdx===3?'var(--red)':'var(--faint)';
    var barCol=rowPct>=100?'var(--green)':rowPct>=60?'var(--blue)':'var(--yellow)';
    var rr=redRows[c.id]||{},rc=redClosed[c.id]||{};
    var openRR=(rr.rows||0)-(rc.rows||0);
    return '<div class="lc s'+c.statusIdx+'" onclick="openLineDetail(\''+c.id+'\')" style="cursor:pointer;padding:12px 14px">'+
      '<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px">'+
        '<div style="display:flex;align-items:center;gap:8px">'+
          ...
          '<span style="font-size:15px;font-weight:800">'+l.name+'</span>'+
        '</div>'+
        '<button data-lid="'+c.id+'" onclick="event.stopPropagation();quickUpdate(this.dataset.lid)" style="font-size:13px;padding:6px 14px;background:var(--blue);border:none;border-radius:7px;color:#fff;cursor:pointer;font-weight:700">עדכן</button>'+
      '</div>'+
      '<div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:6px;margin-bottom:8px">'+
        '<div class="ni"><div class="ni-v" style="font-size:24px;font-weight:900;color:var(--blue)">'+(c.combined||'-')+'</div><div class="ni-l">'+(c.rrU>0?'יעד+אדום':'יעד')+'</div></div>'+
        '<div class="ni"><div class="ni-v" style="font-size:24px;font-weight:900;color:var(--faint)">'+(c.combCum||'-')+'</div><div class="ni-l">תכנון</div></div>'+
        '<div class="ni"><div class="ni-v" style="font-size:28px;font-weight:900;color:var(--green)">'+(c.done!==null?c.done:'-')+'</div><div class="ni-l">בוצע</div></div>'+
      '</div>'+
      (rowQty>0?'<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:4px">'+
        '<span style="font-size:11px;color:var(--dim)">תוכנית עבודה</span>'+
        '<span style="font-size:12px;font-weight:700;font-family:var(--mono);color:'+barCol+'">'+rowDone+'/'+rowQty+' ('+rowPct+'%)</span>'+
      '</div>'+
      '<div style="height:4px;background:var(--border);border-radius:2px;overflow:hidden;margin-bottom:6px">'+
        '<div style="height:100%;background:'+barCol+';width:'+Math.min(rowPct,100)+'%"></div>'+
      '</div>':'') +
      '<div style="display:flex;justify-content:space-between;align-items:center">'+
        '<div style="font-size:11px;color:'+(c.last?'var(--dim)':'var(--red)')+'">'+(c.last?'עודכן '+c.last.time:'לא דווח')+'</div>'+
        (openRR>0?'<span style="font-size:11px;color:var(--red);font-weight:700">'+openRR+' שורות אדומות</span>':'')+
      ...
      '</div>'+
      '<div class="pg-track" style="position:relative;margin-top:8px">'+
        '<div class="pg-fill '+(c.statusIdx===1?'fb':c.statusIdx===0?'fg':c.statusIdx===2?'fy':c.statusIdx===3?'fr':'fgr')+'" style="width:'+Math.min(c.combined>0&&c.done!==null?Math.round(c.done/c.combined*100):0,100)+'%"></div>'+
...
function parseWorkPlan(rows){
  ...
  rows.forEach(function(row){
    ...
    if(qty>0){
      var isRed=startDate<=today; // started but not today = overdue
