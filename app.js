...
function buildDashInlineRows(openLineId){
  ...
  filtered2.forEach(function(r){
    ...
    if(!isDone){
      ...
      html+='<div id="dr-pinput-'+realIdx+'" style="display:none;align-items:center;gap:8px;margin-top:7px"><input type="number" inputmode="numeric" placeholder="כמה?" id="dr-partial-'+realIdx+'" min="1" max="'+r.qty+'" style="flex:1;background:var(--surface);border:1px solid var(--border);border-radius:8px;padding:7px 12px;color:var(--text);font-size:18px;font-family:var(--mono);text-align:center"><button data-lid="'+openLineId+'" data-idx="'+realIdx+'" onclick="markDashPartial(this.dataset.lid,parseInt(this.dataset.idx))" style="padding:7px 16px;background:var(--yellow);border:none;border-radius:8px;color:#fff;font-size:14px;font-weight:700;cursor:pointer">שמור</button></div>';
    }
    ...
    (orderRows[l.id]||[]).forEach(function(r){
      if(r.status==='done'&&r.completedAt){
        ...
        if(!isNaN(hh)&&hourlyData[l.id][hh]!==undefined){
          hourlyData[l.id][hh]+=(r.done||r.qty||0);
        }
      } else if(r.status==='partial'&&r.completedAt){
        var hh2=parseInt(r.completedAt.split(':')[0]);
        if(!isNaN(hh2)&&hourlyData[l.id][hh2]!==undefined){
          hourlyData[l.id][hh2]+=(r.done||0);
        }
      }
    });
  });
