...
function renderSetupScreen(){
  ...
  var tabsHtml='<div style="display:flex;gap:6px;margin-bottom:14px">'+
    '<button onclick="toggleSetupSubTab(\'requests\')" style="flex:1;padding:9px;border-radius:8px;border:1px solid var(--border);font-size:12px;font-weight:700;cursor:pointer;'+
      (window._setupSubTab==='requests'?'background:var(--blue);color:#fff;border-color:var(--blue)':'background:var(--card);color:var(--dim)')+'">\u05d4\u05d6\u05de\u05e0\u05d5\u05ea</button>'+
...
function renderSetupRequests(){
  ...
  var urgent=open.filter(function(r){return waitMin(r)>=SETUP_URGENT_MIN;}).sort(function(a,b){return waitMin(b)-waitMin(a);});
  var waiting=open.filter(function(r){return waitMin(r)<SETUP_URGENT_MIN;}).sort(function(a,b){return waitMin(b)-waitMin(a);});
  ...
  function reqCardButtons(r){
    return '<div style="display:flex;gap:5px" id="setup-btns-'+r.id+'">'+
      ...
      '<div onclick="toggleSetupPartial(\''+r.id+'\')" style="flex:1;text-align:center;padding:7px 3px;background:var(--yellow-bg);border:1px solid rgba(217,119,6,.3);border-radius:6px;color:var(--yellow);font-size:11px;font-weight:700;cursor:pointer">\u05d7\u05dc\u05e7\u05d9</div>'+
    ...
    '</div>'+
    '<div id="setup-partial-'+r.id+'" style="display:none;margin-top:8px;background:var(--yellow-bg);border-radius:8px;padding:9px">'+
  ...
  function reqCard(r,urgentLevel){
    var w=waitMin(r);
    var tCol=urgentLevel?'var(--red)':'var(--yellow)';
    return '<div style="background:var(--card);border:1px solid var(--border);border-radius:10px;padding:10px 12px;margin-bottom:8px">'+
      '<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px">'+
        ...
        '<div style="font-size:15px;font-weight:800;color:'+tCol+';font-family:var(--mono)">'+w+' \u05d3\u05e7</div>'+
      '</div>'+
      reqCardButtons(r)+
    '</div>';
  }
  ...
  if(urgent.length){
    ...
    html+='<div style="font-size:10.5px;font-weight:700;color:var(--red);margin-bottom:8px">\u26a0 \u05d3\u05d7\u05d5\u05e3 \u2014 \u05de\u05de\u05ea\u05d9\u05df \u05de\u05e2\u05dc '+SETUP_URGENT_MIN+' \u05d3\u05e7\u05d5\u05ea</div>';
    urgent.forEach(function(r){html+=reqCard(r,true);});
    html+='</div>';
  }
  if(waiting.length){
    ...
    html+='<div style="font-size:10.5px;font-weight:700;color:var(--yellow);margin-bottom:8px">\u05de\u05de\u05ea\u05d9\u05df \u2014 \u05e4\u05d7\u05d5\u05ea \u05de-'+SETUP_URGENT_MIN+' \u05d3\u05e7\u05d5\u05ea</div>';
    waiting.forEach(function(r){html+=reqCard(r,false);});
    html+='</div>';
  }
  if(!urgent.length&&!waiting.length){
    html+='<div class="empty">\u05d0\u05d9\u05df \u05d1\u05e7\u05e9\u05d5\u05ea \u05e4\u05ea\u05d5\u05d7\u05d5\u05ea \u05db\u05e8\u05d2\u05e2</div>';
  }
  if(handled.length){
