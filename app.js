...
function parseWorkPlan(rows){
  ...
  rows.forEach(function(row){
    ...
    if(qty>0){
      // Compare as date strings to avoid UTC/local timezone issues
      var todayStr2=today.getFullYear()+'-'+('0'+(today.getMonth()+1)).slice(-2)+'-'+('0'+today.getDate()).slice(-2);
      var startStr2=startStr.substring(0,10); // get YYYY-MM-DD part
      var isRed=startStr2<todayStr2; // red only if start date is BEFORE today
