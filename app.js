...
function parseWorkPlan(rows){
  ...
  rows.forEach(function(row){
    ...
    var sku=row['מקԂ7ט']||row['מק"ט']||row['מקט']||'';
    var desc=row['תאור מוצר']||'';
    var orderNum=row['הזמנה']||'';
    var orderLine=row['שורה']||'';
    var customer=row['שם לקוח']||'';
    var expiry=row['פקԂ7ע']||row['פק"ע']||'';
    var skuB=row['יחԂ7 הנעה (בן)']||row['מק"ט בן']||row['מקט בן']||'';
    ...
    if(qty>0){
      ...
      if(s.indexOf('T')>0){
        // ISO format: 2026-04-19T07:48:00.000Z
        startNorm=s.substring(0,10);
      } else if(s.indexOf('/')>0){
        // DD/MM/YYYY format
        var parts=s.substring(0,10).split('/');
        startNorm=parts.length===3?parts[2]+'-'+parts[1]+'-'+parts[0]:'';
      } else if(s.indexOf('-')>0){
        // Already YYYY-MM-DD
        startNorm=s.substring(0,10);
      }
...
function recalcRedRows(){
  ...
  Object.keys(orderRows).forEach(function(lineId){
    (orderRows[lineId]||[]).forEach(function(r){
      ...
      else if(sd.indexOf('/')>0){var p=sd.substring(0,10).split('/');norm=p.length===3?p[2]+'-'+p[1]+'-'+p[0]:'';}
      else norm=sd.substring(0,10);
