...
function syncFromServer(){
  ...
  sbFetch('ops_data?select=key,value').then(function(rows){
    ...
    saveState();
    // Only re-render if user hasn't interacted recently (5 second grace period)
    var _now=Date.now();
    var _recentInteraction=window._lastUserAction&&(_now-window._lastUserAction<5000);
    if(!_recentInteraction){
