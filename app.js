...
var targets={},reports={},reportsUpdatedAt={},orderRowsLastTotal={},redRows={},redClosed={},stoppages=[],alertsLog=[],history={},dayResetMarker=0;
...
function clearDay(){
  ...
  targets={};reports={};reportsUpdatedAt={};orderRowsLastTotal={};redRows={};redClosed={};stoppages=[];palletRequests=[];orderRows={};
...
function applyRemoteResetIfNewer(remoteMarkerStr){
  ...
  if(remoteMarker>dayResetMarker){
    targets={};reports={};reportsUpdatedAt={};orderRowsLastTotal={};redRows={};redClosed={};stoppages=[];orderRows={};
    store.removeItem('ops_order_rows');
...
function renderSetupPlan(){
  ...
  PLAN_LINES.forEach(function(pl){
    ...
    html+='<div style="font-size:11.5px;font-weight:700;color:var(--text);margin-bottom:6px;display:flex;justify-content:space-between"><span>'+pl.name+'</span><span style="color:var(--faint);font-weight:400">'+keys.length+' \u05de\u05e7"\u05d8\u05d9\u05dd</span></div>';
    if(!keys.length){
      html+='<div style="background:var(--card);border:1px solid var(--border);border-radius:10px;padding:10px;text-align:center;font-size:11px;color:var(--faint)">\u05d0\u05d9\u05df \u05e0\u05ea\u05d5\u05e0\u05d9\u05dd</div>';
    } else {
      anyData=true;
      html+='<div style="background:var(--card);border:1px solid var(--border);border-radius:10px;overflow:hidden">';
      keys.sort(function(a,b){return groups[b]-groups[a];}).forEach(function(k,i){
        html+='<div style="display:flex;justify-content:space-between;padding:8px 10px;'+(i<keys.length-1?'border-bottom:1px solid var(--border)':'')+'">'+
          '<span style="font-size:12px;color:var(--text);font-family:var(--mono)">'+escapeHtml(k)+'</span>'+
          '<span style="font-size:13px;font-weight:800;color:var(--text);font-family:var(--mono)">'+groups[k]+'</span>'+
        '</div>';
      });
      html+='</div>';
    }
