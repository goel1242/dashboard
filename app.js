...
// SAFE STUBS - prevent crashes if functions not yet loaded
if(typeof checkMaterialAlerts==='undefined') var checkMaterialAlerts=function(){};
if(typeof updateMatBdg==='undefined') var updateMatBdg=function(){};
if(typeof saveMaterials==='undefined') var saveMaterials=function(){};
if(typeof renderMaterials==='undefined') var renderMaterials=function(){};
if(typeof buildRequestForms==='undefined') var buildRequestForms=function(){};
if(typeof renderPallets==='undefined') var renderPallets=function(){};
if(typeof renderPackRequests==='undefined') var renderPackRequests=function(){};
...
function syncFromServer(){
  ...
  // Don't sync if user is actively filling request form
  var reqQty=document.getElementById('req-qty');
  var reqPallet=document.getElementById('req-pallet');
  if(reqQty&&document.activeElement&&
     (document.activeElement.id==='req-qty'||
      document.activeElement.id==='req-note'||
      document.activeElement.id==='mat-sku'||
      document.activeElement.id==='mat-qty'||
      document.activeElement.id==='mat-note')){
    return; // User is typing - skip sync
  }
  sbFetch('ops_data?select=key,value').then(function(rows){
    ...
    try{checkMaterialAlerts();}catch(e){}
    try{updateMatBdg();}catch(e){}
...
setInterval(function(){if(document.getElementById('page-stops').classList.contains('active')) renderStops();updateStopBdg();checkLongStoppages();},30000);
...
function toggleF(name){
  ...
  if(!isOpen){
    ...
    if(name==='workplan'){}
    if(name==='mathist') renderMaterialHistory();
    if(name==='perfchart') renderPerformanceChart();
    if(name==='stopssummary') renderStopsSummary();
    if(name==='dailyanalysis'){renderMorningPriorities();renderCumulativeGap();renderPatterns();renderWorkforceFocus();}
    if(name==='reports'){renderPerformanceChart();renderRedRowsReport();renderStopsSummary();}
    if(name==='histweekly'){renderWeeklyReport();renderHistory();}
    if(name==='priorities') renderMorningPriorities();
    if(name==='cumulativegap') renderCumulativeGap();
    if(name==='patterns') renderPatterns();
    if(name==='workforcefocus') renderWorkforceFocus();
    if(name==='actionlog') renderActionLog();
    if(name==='redrows') renderRedRowsReport();
    if(name==='productivity') renderProductivity();
...
function renderDash(){
  ...
  // k-gap removed //totGap>=0?('+'+totGap):totGap||'-';
  // Remaining units
  var totDone=0,totCombined=0;
  LINES.forEach(function(l){var c=calcLine(l.id);totDone+=c.done||0;totCombined+=c.combined||0;});
  var totRemaining=Math.max(0,totCombined-totDone);
  // ETA based on current rate
  var elH=Math.max(0.1,elapsed());
  var rate=elH>0?totDone/elH:0;
  var etaEl=document.getElementById('k-eta');
  if(etaEl){
    if(rate>0&&totRemaining>0){
      var etaHrs=totRemaining/rate;
      var etaTime=shiftStart()+elH+etaHrs;
      etaEl.textContent=pad(Math.floor(etaTime))+':'+pad(Math.round((etaTime%1)*60));
    } else etaEl.textContent='-';
  }
  var remEl=document.getElementById('k-rem');
  if(remEl) remEl.textContent=totRemaining||'-';
  document.getElementById('k-ach').textContent=totDone||'-';
  ...
  list.innerHTML=calcs.map(function(c){
    var l=LINES.filter(function(x){return x.id===c.id;})[0];
    var pp=c.combined>0&&c.done!==null?Math.min(Math.round(c.done/c.combined*100),100):0;
    var fc=c.statusIdx===1?'fb':c.statusIdx===0?'fg':c.statusIdx===2?'fy':c.statusIdx===3?'fr':'fgr';
    var dotCol=c.statusIdx===0||c.statusIdx===1?'var(--green)':c.statusIdx===2?'var(--yellow)':'var(--red)';
    if(c.target===0) dotCol='var(--faint)';
    var rr=redRows[c.id]||null,rc2=redClosed[c.id]||null;
    var openR=rr?rr.rows-(rc2?rc2.rows:0):0,openU=rr?rr.units-(rc2?rc2.units:0):0;
    var rrHtml='';
    if(rr&&rr.rows>0){
      var rrCol=openR<=0?'var(--green)':'var(--red)';
      rrHtml='<div style="display:flex;align-items:center;gap:6px;margin-top:6px;font-size:11px">'+
        '<span style="color:'+rrCol+';font-weight:700">שורות אדומות:</span>'+
        (openR>0
          ? '<span style="color:var(--red)">'+openR+' פתוחות | '+openU+' יח</span>'
          : '<span style="color:var(--green)">נוקו</span>')+
      '</div>';
    }
    var reasonTag=c.last&&c.last.reason?'<span style="font-size:10px;padding:2px 7px;border-radius:5px;background:var(--yellow-bg);color:var(--yellow);border:1px solid rgba(217,119,6,.2)">'+c.last.reason+'</span>':'';
    var cardBg=c.statusIdx===3?'background:rgba(220,38,38,.04);':c.statusIdx===2?'background:rgba(245,158,11,.04);':c.statusIdx===0||c.statusIdx===1?'background:rgba(5,150,105,.04);':'';
    return '<div class="lc s'+c.statusIdx+'" style="'+cardBg+'">'+
      '<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:10px">'+
        '<div style="display:flex;align-items:center;gap:7px">'+
          '<span style="width:9px;height:9px;border-radius:50%;background:'+dotCol+';flex-shrink:0;display:inline-block"></span>'+
          '<span class="lc-name">'+l.name+'</span>'+
          reasonTag+
        '</div>'+
        '<button data-lid="'+c.id+'" onclick="quickUpdate(this.dataset.lid)" style="font-size:14px;padding:8px 18px;background:var(--blue);border:none;border-radius:8px;color:#fff;cursor:pointer;font-weight:700;box-shadow:0 2px 6px rgba(37,99,235,.3)">עדכן</button>'+
      '</div>'+
      '<div style="display:grid;grid-template-columns:repeat(3,1fr);gap:6px;margin-bottom:10px">'+
        '<div class="ni"><div class="ni-v" style="font-size:28px;font-weight:900;color:var(--blue)">'+(c.combined||'-')+'</div><div class="ni-l">'+(c.rrU>0?'יעד+אדום':'יעד יומי')+'</div></div>'+
        '<div class="ni"><div class="ni-v" style="font-size:28px;font-weight:900;color:var(--faint)">'+(c.combCum||'-')+'</div><div class="ni-l">תכנון כעת</div></div>'+
        '<div class="ni"><div class="ni-v" style="font-size:32px;font-weight:900;color:var(--green)">'+(c.done!==null?c.done:'-')+'</div><div class="ni-l">בוצע</div></div>'+
      '</div>'+
      '<div class="pg-track" style="position:relative">'+
        '<div class="pg-fill '+fc+'" style="width:'+Math.min(pp,100)+'%;min-width:'+(pp>0?'3px':'0')+'"></div>'+
        buildHourlyMarkers(c)+
      '</div>'+
      '<div style="display:flex;justify-content:space-between;align-items:center;margin-top:14px">'+
  '<div style="font-size:11px;color:'+(c.last?'var(--dim)':'var(--red)')+';font-family:var(--mono)">'+(c.last?'עודכן '+c.last.time:'לא דווח עדיין')+'</div>'+
  (c.achPct!==null?'<div style="font-size:12px;font-weight:700;color:'+(c.achPct>=95?'var(--green)':c.achPct>=85?'var(--yellow)':'var(--red)')+'">'+c.achPct+'%</div>':'')+
'</div>'+
      rrHtml+
    '</div>';
  }).join('');
...
function openStop(){
  ...
  logAction('stop','פתיחה: '+line+' - '+reason);
...
function quickUpdate(lineId){
  ...
  // Show previous updates history
  var reps=reports[lineId]||[];
  var histEl=document.getElementById('np-history');
  if(histEl){
    if(reps.length){
      var histHtml='<div style="font-size:10px;font-weight:700;color:var(--dim);margin-bottom:4px">עדכונים קודמים:</div>';
      reps.slice().reverse().slice(0,5).forEach(function(r){
        histHtml+='<div style="display:flex;justify-content:space-between;padding:3px 0;border-bottom:1px solid var(--border);font-size:12px">';
        histHtml+='<span style="color:var(--faint);font-family:var(--mono)">'+r.time+'</span>';
        histHtml+='<span style="font-weight:700;font-family:var(--mono)">'+r.done+' יח</span>';
        if(r.reason) histHtml+='<span style="color:var(--dim);font-size:10px">'+r.reason+'</span>';
        histHtml+='</div>';
      });
...
function npSave(){
  ...
  var line=LINES.filter(function(l){return l.id===lineId;})[0];
  logAction('update',(line?line.name:lineId)+': '+done+' יח'+(reason?' ('+reason+')':''));
  document.getElementById('np-modal').classList.remove('open');
  if(!validateUpdate(lineId,done,reason,false)){
    return; // validation modal took over
  }
  saveUpdate(lineId,done,reason,false);
}
...
function submitMaterialRequest(){
  ...
  logAction('material',(line?line.name:'')+': '+sku+' x'+qty);
  showT('בקשה נשלחה!');
  renderMaterials();
}
...
function renderMaterialHistory(){
  var el=document.getElementById('mat-history-mgr');
  if(!el) return;
  var allDone=materialRequests.filter(function(r){return r.status==='arrived'||r.status==='delayed';});
  if(!allDone.length){el.innerHTML='<div class="empty">אין היסטוריה</div>';return;}
  var searchTerm=((document.getElementById('mat-search-mgr')||{}).value||'').toLowerCase();
  var filtered=searchTerm?allDone.filter(function(r){return (r.sku||'').toLowerCase().includes(searchTerm)||(r.lineName||'').toLowerCase().includes(searchTerm)||(r.delayReason||'').toLowerCase().includes(searchTerm);}):allDone;
  ...
  var html='<div style="background:var(--surface);border:1px solid var(--border);border-radius:10px;padding:10px 12px;margin-bottom:8px">';
  ...
  html+='</div>';
  html+='<input type="text" id="mat-search-mgr" placeholder="חפש מק-ט / נתיב / סיבה..." oninput="renderMaterialHistory()" style="width:100%;background:var(--bg);border:1px solid var(--border);border-radius:7px;padding:7px 10px;color:var(--text);font-size:12px">';
  html+='</div>';
...
function parseWorkPlan(rows){
  ...
  var status=document.getElementById('wp-status');
  var totals={pick:{qty:0,orders:0,spillover:0,mins:0},kit:{qty:0,orders:0,spillover:0,mins:0},
    drive:{qty:0,orders:0,spillover:0,mins:0},spk:{qty:0,orders:0,spillover:0,mins:0},cable:{qty:0,orders:0,spillover:0,mins:0}};
  ...
  rows.forEach(function(row){
    ...
    if(startDate.getTime()===today.getTime()){
      ...
      if(endDate>today) totals[lineId].spillover++;
      if(startStr&&endStr){var sf=new Date(startStr),ef=new Date(endStr);var m=(ef-sf)/60000;if(m>0)totals[lineId].mins+=m;}
    }
  });
  if(matched===0){
    if(status){status.textContent='אין הזמנות להיום';status.style.color='var(--yellow)';}
    return;
  }
  ...
  var is2='background:var(--surface);border:1px solid var(--border);border-radius:8px;padding:9px 8px;color:var(--text);font-size:14px;width:100%;cursor:pointer';
  LINES.forEach(function(l){
    ...
    if(d.qty>0||rr.rows>0){
      ...
      if(d.qty>0) html+='<span style="font-size:12px;font-family:var(--mono);font-weight:700;color:var(--green)">'+d.qty+' יח היום</span>';
      if(d.orders>0) html+='<span style="font-size:11px;color:var(--faint)">'+d.orders+' הזמנות</span>';
      if(d.spillover>0) html+='<span style="font-size:10px;color:var(--yellow);background:var(--yellow-bg);padding:2px 6px;border-radius:4px">'+d.spillover+' גולשות</span>';
      if(rr.rows>0) html+='<span style="font-size:10px;color:var(--red);background:var(--red-bg);padding:2px 6px;border-radius:4px;font-weight:700">'+rr.rows+' שורות אדומות ('+rr.units+' יח)</span>';
      if(d.mins>0&&d.qty>0){
        var workers=DEFAULT_WORKERS[l.id]||1;
        var totalMins=Math.round((d.mins/d.qty*(d.qty+rr.units))/workers);
        var shStart=new Date();shStart.setHours(7,15,0,0);
        var etaD=new Date(shStart.getTime()+totalMins*60000);
        var etaStr=('0'+etaD.getHours()).slice(-2)+':'+('0'+etaD.getMinutes()).slice(-2);
        var over=totalMins>shiftHours()*60;
        html+='<span id="wp-eta-'+l.id+'" style="font-size:11px;padding:3px 8px;border-radius:5px;font-weight:700;background:'+(over?'var(--red-bg)':'var(--blue-bg)')+';color:'+(over?'var(--red)':'var(--blue)')+'">ETA '+etaStr+(over?' חריגה':'')+'</span>';
      }
      html+='</div></div>';
    }
  });
  ...
  if(status){status.textContent=matched+' הזמנות נמצאו להיום';status.style.color='var(--green)';}
}

function renderWpEta(){
  if(!wpParsed) return;
  LINES.forEach(function(l){
    var d=wpParsed[l.id],rr=(wpParsed._redRows||{})[l.id]||{rows:0,units:0};
    var el=document.getElementById('wp-eta-'+l.id);
    if(!el||!d||!d.mins||!d.qty) return;
    var workers=DEFAULT_WORKERS[l.id]||1;
    var totalMins=Math.round((d.mins/d.qty*(d.qty+rr.units))/workers);
    var shStart=new Date();shStart.setHours(7,15,0,0);
    var etaD=new Date(shStart.getTime()+totalMins*60000);
    var etaStr=('0'+etaD.getHours()).slice(-2)+':'+('0'+etaD.getMinutes()).slice(-2);
    var over=totalMins>shiftHours()*60;
    el.textContent='ETA '+etaStr+(over?' חריגה':'');
    el.style.background=over?'var(--red-bg)':'var(--blue-bg)';
    el.style.color=over?'var(--red)':'var(--blue)';
  });
}

function updateWpWorkers(el){var id=el.id.replace('wp-wk-','');DEFAULT_WORKERS[id]=parseInt(el.value)||1;saveState();renderWpEta();}

// SKU MEMORY
function getKnownSkus(){try{return JSON.parse(store.getItem('ops_skus')||'[]');}catch(e){return [];}}
function saveSku(sku){if(!sku)return;var skus=getKnownSkus();if(!skus.includes(sku)){skus.unshift(sku);if(skus.length>700)skus=skus.slice(0,700);store.setItem('ops_skus',JSON.stringify(skus));}}
...
function deleteSku(sku){
  var skus=getKnownSkus().filter(function(s){return s!==sku;});
  store.setItem('ops_skus',JSON.stringify(skus));renderSkuManager();
}
...
function startMatTimer(){
  if(typeof matTimerInterval!=='undefined'&&matTimerInterval) clearInterval(matTimerInterval);
...
// ============================================================
// 1. Edit active stoppage reason
// ============================================================
function editActiveStop(id){
  var s=stoppages.filter(function(x){return x.id===id;})[0];
  if(!s) return;
  var newReason=document.getElementById('edit-reason-'+id);
  if(!newReason||!newReason.value) return;
  s.reason=newReason.value;
  saveState();syncToServer();renderStops();showT('סיבה עודכנה');
}

// ============================================================
// 2. Alert for long active stoppages (>30 min)
// ============================================================
function checkLongStoppages(){
  stoppages.filter(function(s){return s.open;}).forEach(function(s){
    var min=Math.round((Date.now()-s.startTs)/60000);
    if(min===30||min===60){
      pushAlert('r','עצירה ארוכה: '+s.lineName+' - '+s.reason+' ('+min+' min)');
      showT('עצירה פעילה '+min+' דקות: '+s.lineName,'r');
    }
  });
}

// ============================================================
// 3. Performance vs plan chart (visual)
// ============================================================
function renderPerformanceChart(){
  var el=document.getElementById('perf-chart');
  if(!el) return;
  var sh=shiftHours(),startH=7.25;
  var now=new Date();
  var elapsedH=Math.max(0,(now.getHours()+now.getMinutes()/60)-startH);
  var totalHours=Math.floor(sh);
  
  // Collect data per line per hour
  var html='';
  LINES.forEach(function(l){
    if(!targets[l.id]) return;
    var reps=reports[l.id]||[];
    if(!reps.length) return;
    var target=targets[l.id]+((redRows[l.id]||{}).units||0);
    var hourlyTarget=target/sh;
    
    // Build SVG chart
    var w=280,h=80,pad=8;
    var maxVal=target;
    var points_plan=[],points_actual=[];
    
    for(var hr=0;hr<=totalHours;hr++){
      var px=pad+(hr/totalHours)*(w-pad*2);
      var planY=h-pad-((hourlyTarget*hr)/maxVal)*(h-pad*2);
      points_plan.push(px+','+planY);
    }
    
    reps.forEach(function(r,i){
      var px=pad+((i+1)/totalHours*(w-pad*2));
      var py=h-pad-(r.done/maxVal)*(h-pad*2);
      points_actual.push(px+','+py);
    });
    
    var planColor='3B82F6',actualColor=reps[reps.length-1]&&reps[reps.length-1].done>=hourlyTarget*elapsedH?'10B981':'EF4444';
    
    html+='<div style="background:var(--card);border:1px solid var(--border);border-radius:10px;padding:10px;margin-bottom:8px">';
    html+='<div style="display:flex;justify-content:space-between;margin-bottom:6px">';
    html+='<span style="font-size:13px;font-weight:700">'+l.name+'</span>';
    html+='<span style="font-size:11px;color:var(--dim);font-family:var(--mono)">יעד: '+target+'</span>';
    html+='</div>';
    
    if(points_actual.length>0){
      html+='<svg width="100%" viewBox="0 0 '+w+' '+h+'" style="overflow:visible">';
      // Grid lines
      for(var g=1;g<=4;g++){
        var gy=pad+(g/4)*(h-pad*2);
        html+='<line x1="'+pad+'" y1="'+gy+'" x2="'+(w-pad)+'" y2="'+gy+'" stroke="#E2E8F0" stroke-width="0.5"/>';
      }
      // Plan line
      html+='<polyline points="'+points_plan.join(' ')+'" fill="none" stroke="#'+planColor+'" stroke-width="1.5" stroke-dasharray="4,3" opacity="0.6"/>';
      // Actual line
      html+='<polyline points="'+points_actual.join(' ')+'" fill="none" stroke="#'+actualColor+'" stroke-width="2"/>';
      // Dots on actual
      points_actual.forEach(function(p){
        var xy=p.split(',');
        html+='<circle cx="'+xy[0]+'" cy="'+xy[1]+'" r="3" fill="#'+actualColor+'"/>';
      });
      // Legend
      html+='<line x1="'+pad+'" y1="'+(h-2)+'" x2="'+(pad+16)+'" y2="'+(h-2)+'" stroke="#'+planColor+'" stroke-width="1.5" stroke-dasharray="4,3"/>';
      html+='<text x="'+(pad+20)+'" y="'+(h)+'" font-size="8" fill="#64748B">תכנון</text>';
      html+='<line x1="'+(pad+60)+'" y1="'+(h-2)+'" x2="'+(pad+76)+'" y2="'+(h-2)+'" stroke="#'+actualColor+'" stroke-width="2"/>';
      html+='<text x="'+(pad+80)+'" y="'+(h)+'" font-size="8" fill="#'+actualColor+'">בפועל</text>';
      html+='</svg>';
    } else {
      html+='<div style="font-size:11px;color:var(--faint);text-align:center;padding:10px">ממתין לעדכונים</div>';
    }
    html+='</div>';
  });
  
  if(!html) html='<div class="empty">טרם הוזנו יעדים ועדכונים</div>';
  el.innerHTML=html;
}

// ============================================================
// 4. Red rows over time report
// ============================================================
function renderRedRowsReport(){
  var el=document.getElementById('red-rows-report');
  if(!el) return;
  
  // Current day
  var todayTotal=0,todayOpen=0,todayLines=[];
  LINES.forEach(function(l){
    var rr=redRows[l.id]||{},rc=redClosed[l.id]||{};
    if(rr.rows>0){
      todayTotal+=rr.rows;
      var open=rr.rows-(rc.rows||0);
      todayOpen+=open;
      todayLines.push({name:l.name,total:rr.rows,open:open,closed:rc.rows||0});
    }
  });
  
  // Historical
  var histData=[];
  Object.keys(history).sort().reverse().slice(0,14).forEach(function(dk){
    var d=history[dk];
    var dayTotal=0,dayOpen=0;
    LINES.forEach(function(l){
      var rr=(d.redRows||{})[l.id]||{};
      if(rr.rows) dayTotal+=rr.rows;
    });
    if(dayTotal>0) histData.push({date:new Date(dk).toLocaleDateString('he-IL'),total:dayTotal});
  });
  ...
  html+='<div style="font-size:12px;font-weight:700;color:var(--dim);text-transform:uppercase;letter-spacing:.5px;margin-bottom:8px">היום</div>';
  html+='<div class="sgrid2" style="margin-bottom:8px">';
  html+='<div class="sbox"><div class="sbox-v" style="font-size:20px;color:var(--red)">'+todayTotal+'</div><div class="sbox-l">שורות אדומות</div></div>';
  html+='<div class="sbox"><div class="sbox-v" style="font-size:20px;color:'+(todayOpen>0?'var(--red)':'var(--green)')+'">'+todayOpen+'</div><div class="sbox-l">פתוחות</div></div>';
  html+='</div>';
  if(todayLines.length){
    todayLines.forEach(function(l){
      ...
      html+='<div style="display:flex;gap:10px"><span style="color:var(--red)">'+l.total+' שורות</span><span style="color:'+(l.open>0?'var(--yellow)':'var(--green)')+'">'+l.open+' פתוחות</span></div>';
      html+='</div>';
    });
  } else {
    html+='<div style="font-size:12px;color:var(--green);text-align:center">אין שורות אדומות היום</div>';
  }
  html+='</div>';
  ...
  if(histData.length){
    ...
    html+='<div style="font-size:12px;font-weight:700;color:var(--dim);text-transform:uppercase;letter-spacing:.5px;margin-bottom:8px">היסטוריה (14 ימים)</div>';
    histData.forEach(function(d){
      var w=Math.min(Math.round(d.total/10*100),100);
      html+='<div style="display:flex;align-items:center;gap:8px;margin-bottom:6px">';
      html+='<span style="font-size:10px;color:var(--faint);font-family:var(--mono);min-width:60px">'+d.date+'</span>';
      html+='<div style="flex:1;height:14px;background:var(--border);border-radius:3px;overflow:hidden">';
      html+='<div style="height:100%;background:var(--red-bg);border-right:2px solid var(--red);width:'+w+'%"></div></div>';
      html+='<span style="font-size:11px;color:var(--red);font-weight:700;min-width:20px">'+d.total+'</span>';
      html+='</div>';
    });
    html+='</div>';
  }
...
// STOPS SUMMARY - daily by line + history
function renderStopsSummary(){
  var el=document.getElementById('stops-summary');
  if(!el) return;

  // Today summary by line
  var byLine={};
  LINES.forEach(function(l){byLine[l.name]={count:0,mins:0,reasons:{}};});
  byLine['אריזה']={count:0,mins:0,reasons:{}};

  stoppages.filter(function(s){return !s.open;}).forEach(function(s){
    if(!byLine[s.lineName]) byLine[s.lineName]={count:0,mins:0,reasons:{}};
    byLine[s.lineName].count++;
    byLine[s.lineName].mins+=s.durationMin||0;
    var r=s.reason||'אחר';
    byLine[s.lineName].reasons[r]=(byLine[s.lineName].reasons[r]||0)+1;
  });

  var activeCount=stoppages.filter(function(s){return s.open;}).length;
  var totCount=stoppages.filter(function(s){return !s.open;}).length;
  var totMins=stoppages.filter(function(s){return !s.open;}).reduce(function(s,x){return s+(x.durationMin||0);},0);

  var html='';

  // KPI strip
  html+='<div class="sgrid3" style="margin-bottom:12px">';
  html+='<div class="sbox"><div class="sbox-v" style="color:var(--red)">'+activeCount+'</div><div class="sbox-l">פעילות עכשיו</div></div>';
  html+='<div class="sbox"><div class="sbox-v">'+totCount+'</div><div class="sbox-l">סה"כ היום</div></div>';
  html+='<div class="sbox"><div class="sbox-v" style="color:var(--yellow)">'+totMins+'</div><div class="sbox-l">דקות סה"כ</div></div>';
  html+='</div>';

  // By line
  var linesWithStops=Object.keys(byLine).filter(function(k){return byLine[k].count>0;});
  if(linesWithStops.length){
    html+='<div style="font-size:11px;font-weight:700;color:var(--dim);text-transform:uppercase;letter-spacing:.5px;margin-bottom:8px">פילוח לפי נתיב</div>';
    linesWithStops.forEach(function(lName){
      var d=byLine[lName];
      var topR=Object.entries(d.reasons).sort(function(a,b){return b[1]-a[1];}).slice(0,2);
      html+='<div style="background:var(--card);border:1px solid var(--border);border-radius:10px;padding:10px 12px;margin-bottom:7px">';
      html+='<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:6px">';
      html+='<span style="font-size:13px;font-weight:700">'+lName+'</span>';
      html+='<div style="display:flex;gap:10px">';
      html+='<span style="font-size:12px;font-family:var(--mono);color:var(--red)">'+d.count+' אירועים</span>';
      html+='<span style="font-size:12px;font-family:var(--mono);color:var(--yellow)">'+d.mins+' min</span>';
      html+='</div></div>';
      if(topR.length){
        html+='<div style="display:flex;flex-wrap:wrap;gap:5px">';
        topR.forEach(function(r){
          html+='<span style="font-size:10px;padding:2px 8px;border-radius:12px;background:var(--yellow-bg);color:var(--yellow);border:1px solid rgba(217,119,6,.2)">'+r[0]+' ('+r[1]+'x)</span>';
        });
        html+='</div>';
      }
      html+='</div>';
    ...
    html+='<div style="font-size:13px;color:var(--green);text-align:center;padding:12px">אין עצירות היום</div>';
  }

  // Historical stops
  var histStops=[];
  Object.keys(history).sort().reverse().slice(0,7).forEach(function(dk){
    var d=history[dk];
    var dayStops=(d.stoppages||[]).filter(function(s){return !s.open;});
    if(dayStops.length){
      var dayMins=dayStops.reduce(function(s,x){return s+(x.durationMin||0);},0);
      var byR={};
      dayStops.forEach(function(s){byR[s.reason]=(byR[s.reason]||0)+1;});
      var topR=Object.entries(byR).sort(function(a,b){return b[1]-a[1];})[0];
      histStops.push({date:new Date(dk).toLocaleDateString('he-IL'),count:dayStops.length,mins:dayMins,topReason:topR?topR[0]:''});
    }
  });

  if(histStops.length){
    html+='<div style="font-size:11px;font-weight:700;color:var(--dim);text-transform:uppercase;letter-spacing:.5px;margin:12px 0 8px">היסטוריה שבועית</div>';
    histStops.forEach(function(d){
      ...
      html+='<div>';
      html+='<span style="font-size:12px;font-weight:600">'+d.date+'</span>';
      if(d.topReason) html+=' <span style="font-size:10px;color:var(--dim)">| '+d.topReason+'</span>';
      html+='</div>';
      html+='<div style="display:flex;gap:10px">';
      html+='<span style="font-size:12px;font-family:var(--mono);color:var(--red)">'+d.count+' x</span>';
      html+='<span style="font-size:12px;font-family:var(--mono);color:var(--yellow)">'+d.mins+' min</span>';
      html+='</div></div>';
    });
  }
...
// ============================================================
// MORNING PRIORITIES - what to focus on today
// ============================================================
function renderMorningPriorities(){
  var el=document.getElementById('morning-priorities');
  if(!el) return;
  ...
  LINES.forEach(function(l){
    ...
    // Red rows penalty
    var rr=redRows[l.id]||{},rc=redClosed[l.id]||{};
    var openRR=rr.rows?(rr.rows-(rc.rows||0)):0;
    if(openRR>0){score+=30;reasons.push(openRR+' שורות אדומות פתוחות');}

    // Performance gap
    if(calc.achPct!==null&&calc.achPct<85){score+=25;reasons.push('ביצוע '+calc.achPct+'%');}
    else if(calc.achPct!==null&&calc.achPct<95){score+=10;reasons.push('בסיכון '+calc.achPct+'%');}

    // Cumulative gap
    var gap=calc.gap||0;
    if(gap<-50){score+=25;reasons.push('פיגור '+Math.abs(gap)+' יח');}
    else if(gap<-20){score+=10;reasons.push('פיגור '+Math.abs(gap)+' יח');}

    // Historical - how often this line underperforms
    var histCount=0,histTotal=0;
    Object.keys(history).slice(-5).forEach(function(dk){
      var d=history[dk];
      if(d.reports&&d.reports[l.id]&&d.targets&&d.targets[l.id]){
        histTotal++;
        var lastRep=d.reports[l.id];
        var lastDone=lastRep[lastRep.length-1]?lastRep[lastRep.length-1].done:0;
        if(lastDone<d.targets[l.id]*0.9) histCount++;
      }
    });
    if(histTotal>0&&histCount/histTotal>0.6){score+=15;reasons.push('פיגר '+histCount+'/'+histTotal+' ימים אחרונים');}

    scores.push({id:l.id,name:l.name,score:score,reasons:reasons,calc:calc});
  });

  scores.sort(function(a,b){return b.score-a.score;});
  var top=scores.slice(0,3).filter(function(s){return s.score>0;});

  var html='';
  if(!top.length){
    html='<div style="background:var(--green-bg);border:1px solid rgba(5,150,105,.2);border-radius:10px;padding:14px;text-align:center">';
    html+='<div style="font-size:16px;font-weight:700;color:var(--green)">כל הנתיבים במצב טוב!</div>';
    html+='<div style="font-size:12px;color:var(--dim);margin-top:4px">אין נתיבים בסיכון כרגע</div></div>';
  } else {
    top.forEach(function(s,i){
      var col=i===0?'var(--red)':i===1?'var(--yellow)':'var(--blue)';
      var bg=i===0?'var(--red-bg)':i===1?'var(--yellow-bg)':'var(--blue-bg)';
      var bdr=i===0?'rgba(220,38,38,.25)':i===1?'rgba(217,119,6,.25)':'rgba(37,99,235,.25)';
      var priority=i===0?'עדיפות ראשונה':i===1?'עדיפות שנייה':'עדיפות שלישית';
      html+='<div style="background:'+bg+';border:1px solid '+bdr+';border-radius:10px;padding:12px;margin-bottom:8px">';
      html+='<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:6px">';
      html+='<div style="display:flex;align-items:center;gap:8px">';
      html+='<span style="font-size:11px;font-weight:700;padding:2px 8px;background:'+col+';color:#fff;border-radius:12px">'+priority+'</span>';
      html+='<span style="font-size:15px;font-weight:800">'+s.name+'</span>';
      html+='</div>';
      html+='<span style="font-size:20px;font-weight:900;font-family:var(--mono);color:'+col+'">'+s.score+'</span>';
      html+='</div>';
      if(s.reasons.length){
        html+='<div style="display:flex;flex-wrap:wrap;gap:5px">';
        s.reasons.forEach(function(r){
          html+='<span style="font-size:11px;padding:2px 8px;border-radius:6px;background:var(--surface);color:var(--text)">'+r+'</span>';
        });
        html+='</div>';
      }
      html+='</div>';
...
// ============================================================
// CUMULATIVE GAP - how many units behind plan
// ============================================================
function renderCumulativeGap(){
  var el=document.getElementById('cumulative-gap');
  if(!el) return;

  var totalGap=0,totalDone=0,totalPlan=0;
  var lineGaps=[];

  LINES.forEach(function(l){
    if(!targets[l.id]) return;
    var calc=calcLine(l.id);
    if(calc.done===null) return;
    var gap=(calc.done||0)-(calc.combCum||0);
    totalGap+=gap;
    totalDone+=calc.done||0;
    totalPlan+=calc.combCum||0;
    lineGaps.push({name:l.name,done:calc.done,plan:calc.combCum||0,gap:gap});
  });

  var html='';
  // Main KPI
  var gapColor=totalGap>=0?'var(--green)':totalGap>=-50?'var(--yellow)':'var(--red)';
  html+='<div style="background:var(--card);border:1px solid var(--border);border-radius:12px;padding:16px;margin-bottom:10px;text-align:center">';
  html+='<div style="font-size:13px;color:var(--dim);margin-bottom:4px">פיגור/קידום מצטבר</div>';
  html+='<div style="font-size:52px;font-weight:900;font-family:var(--mono);color:'+gapColor+'">'+(totalGap>=0?'+':'')+totalGap+'</div>';
  html+='<div style="font-size:12px;color:var(--dim)">יחידות ביחס לתכנון שעתי</div>';
  html+='</div>';

  // Per line bars
  lineGaps.forEach(function(l){
    var pct=l.plan>0?Math.round(l.done/l.plan*100):0;
    var barColor=pct>=100?'var(--green)':pct>=85?'var(--yellow)':'var(--red)';
    html+='<div style="margin-bottom:8px">';
    html+='<div style="display:flex;justify-content:space-between;font-size:12px;margin-bottom:3px">';
    html+='<span style="font-weight:600">'+l.name+'</span>';
    html+='<span style="font-family:var(--mono);color:'+(l.gap>=0?'var(--green)':'var(--red)')+'">'+(l.gap>=0?'+':'')+l.gap+' יח</span>';
    html+='</div>';
    html+='<div style="height:8px;background:var(--border);border-radius:4px;overflow:hidden">';
    html+='<div style="height:100%;background:'+barColor+';width:'+Math.min(pct,100)+'%;transition:width .3s"></div>';
    html+='</div>';
    html+='<div style="font-size:10px;color:var(--faint);margin-top:2px">'+l.done+' / '+l.plan+' ('+pct+'%)</div>';
    html+='</div>';
  });
...
// ============================================================
// RECURRING PATTERNS - what repeats
// ============================================================
function renderPatterns(){
  var el=document.getElementById('recurring-patterns');
  if(!el) return;

  // Analyze last 7 days history
  var stopReasons={},lineIssues={},matReasons={};

  Object.keys(history).sort().reverse().slice(0,7).forEach(function(dk){
    var d=history[dk];
    (d.stoppages||[]).filter(function(s){return !s.open;}).forEach(function(s){
      stopReasons[s.reason]=(stopReasons[s.reason]||0)+1;
      lineIssues[s.lineName]=(lineIssues[s.lineName]||0)+(s.durationMin||0);
    });
    (d.materials||[]).filter(function(r){return r.status==='delayed';}).forEach(function(r){
      var reason=(r.delayReasons||[]).map(function(x){return x.reason;}).join(',')||r.delayReason||'';
      if(reason) matReasons[reason]=(matReasons[reason]||0)+1;
    });
  });
  ...
  materialRequests.filter(function(r){return r.status==='delayed';}).forEach(function(r){
    var reason=(r.delayReasons||[]).map(function(x){return x.reason;}).join(',')||r.delayReason||'';
    if(reason) matReasons[reason]=(matReasons[reason]||0)+1;
  });

  var topStopReasons=Object.entries(stopReasons).sort(function(a,b){return b[1]-a[1];}).slice(0,3);
  var topLineIssues=Object.entries(lineIssues).sort(function(a,b){return b[1]-a[1];}).slice(0,3);
  var topMatReasons=Object.entries(matReasons).sort(function(a,b){return b[1]-a[1];}).slice(0,2);

  var html='';
  if(!topStopReasons.length&&!topMatReasons.length){
    html='<div class="empty">אין מספיק נתונים עדיין</div>';
    el.innerHTML=html;return;
  }

  if(topStopReasons.length){
    html+='<div style="font-size:11px;font-weight:700;color:var(--dim);text-transform:uppercase;letter-spacing:.5px;margin-bottom:8px">סיבות עצירה חוזרות (7 ימים)</div>';
    topStopReasons.forEach(function(r,i){
      var w=Math.round(r[1]/topStopReasons[0][1]*100);
      html+='<div style="margin-bottom:6px">';
      html+='<div style="display:flex;justify-content:space-between;font-size:12px;margin-bottom:2px">';
      html+='<span>'+r[0]+'</span><span style="font-family:var(--mono);font-weight:700">'+r[1]+'x</span></div>';
      html+='<div style="height:6px;background:var(--border);border-radius:3px"><div style="height:100%;background:var(--red);border-radius:3px;width:'+w+'%"></div></div>';
      html+='</div>';
    });
  }

  if(topLineIssues.length){
    html+='<div style="font-size:11px;font-weight:700;color:var(--dim);text-transform:uppercase;letter-spacing:.5px;margin:12px 0 8px">נתיבים עם הכי הרבה זמן עצירה</div>';
    topLineIssues.forEach(function(l){
      html+='<div style="display:flex;justify-content:space-between;padding:6px 10px;background:var(--card);border:1px solid var(--border);border-radius:7px;margin-bottom:5px;font-size:12px">';
      html+='<span>'+l[0]+'</span><span style="font-family:var(--mono);color:var(--yellow)">'+l[1]+' min</span></div>';
    });
  }

  if(topMatReasons.length){
    html+='<div style="font-size:11px;font-weight:700;color:var(--dim);text-transform:uppercase;letter-spacing:.5px;margin:12px 0 8px">עיכובי חומר חוזרים</div>';
    topMatReasons.forEach(function(r){
      html+='<div style="display:flex;justify-content:space-between;padding:6px 10px;background:var(--card);border:1px solid var(--border);border-radius:7px;margin-bottom:5px;font-size:12px">';
      html+='<span>'+r[0]+'</span><span style="font-family:var(--mono);color:var(--orange,#F97316)">'+r[1]+'x</span></div>';
    });
  }

  // Insight
  if(topStopReasons.length){
    html+='<div style="background:var(--blue-bg);border:1px solid rgba(37,99,235,.2);border-radius:8px;padding:10px;margin-top:10px;font-size:12px;color:var(--text)">';
    html+='<span style="font-weight:700">תובנה: </span>';
    html+='"'+topStopReasons[0][0]+'" הסיבה המובילה לעצירות ('+topStopReasons[0][1]+' פעמים). כדאי לבדוק מול ';
    html+=(topStopReasons[0][0]==='מחסן'||topStopReasons[0][0]==='לא הוזמן בזמן'?'מנהל המחסן':topStopReasons[0][0]==='הנדסה'||topStopReasons[0][0]==='IT'?'הגורם המקצועי הרלוונטי':'הנהלת הייצור')+'.</div>';
  }
...
// ============================================================
// WORKFORCE FOCUS - where to move people
// ============================================================
function renderWorkforceFocus(){
  var el=document.getElementById('workforce-focus');
  if(!el) return;

  var data=LINES.map(function(l){
    var calc=calcLine(l.id);
    var workers=DEFAULT_WORKERS[l.id]||1;
    if(!targets[l.id]) return null;
    var gap=calc.gap||0;
    var remaining=Math.max(0,(calc.combined||0)-(calc.done||0));
    var elapsedH=Math.max(0.1,elapsed());
    var rate=calc.done>0?calc.done/elapsedH:0;
    var ratePerWorker=workers>0?rate/workers:0;
    var etaHours=ratePerWorker>0&&remaining>0?remaining/ratePerWorker/workers:null;
    var shLeft=shiftHours()-elapsedH;
    var status=etaHours===null?'unknown':etaHours<=shLeft?'ok':'risk';
    return{id:l.id,name:l.name,workers:workers,gap:gap,status:status,etaHours:etaHours,shLeft:shLeft,remaining:remaining};
  }).filter(function(d){return d!==null;});

  var atRisk=data.filter(function(d){return d.status==='risk';});
  var ahead=data.filter(function(d){return d.status==='ok'&&d.gap>20&&d.workers>1;});

  var html='';

  if(!atRisk.length){
    html='<div style="background:var(--green-bg);border:1px solid rgba(5,150,105,.2);border-radius:10px;padding:12px;text-align:center;margin-bottom:10px">';
    html+='<div style="font-size:14px;font-weight:700;color:var(--green)">כל הנתיבים בקצב תקין</div></div>';
  } else {
    html+='<div style="font-size:11px;font-weight:700;color:var(--red);text-transform:uppercase;letter-spacing:.5px;margin-bottom:8px">נדרשת התייחסות</div>';
    atRisk.forEach(function(d){
      var donor=ahead[0];
      html+='<div style="background:var(--red-bg);border:1px solid rgba(220,38,38,.2);border-radius:10px;padding:10px 12px;margin-bottom:7px">';
      html+='<div style="font-size:13px;font-weight:700;margin-bottom:4px">'+d.name+'</div>';
      html+='<div style="font-size:11px;color:var(--dim);margin-bottom:6px">';
      html+='נותרו '+d.remaining+' יח | '+Math.round(d.shLeft*60)+' דקות למשמרת';
      html+='</div>';
      if(donor){
        html+='<div style="font-size:12px;font-weight:600;color:var(--text)">';
        html+='המלצה: העבר עובד מ-<strong>'+donor.name+'</strong> ('+donor.workers+' עובדים, מקדים)';
        html+='</div>';
      } else {
        html+='<div style="font-size:12px;color:var(--text)">המלצה: הגדל כ"א לנתיב זה</div>';
      }
      html+='</div>';
    });
  }
  ...
  // All lines summary
  html+='<div style="font-size:11px;font-weight:700;color:var(--dim);text-transform:uppercase;letter-spacing:.5px;margin:10px 0 8px">מצב כ"א</div>';
  data.forEach(function(d){
    var col=d.status==='ok'?'var(--green)':d.status==='risk'?'var(--red)':'var(--faint)';
    ...
    html+='<span style="font-size:12px;font-weight:600">'+d.name+'</span>';
    html+='<div style="display:flex;align-items:center;gap:10px">';
    html+='<span style="font-size:11px;color:var(--dim)">'+d.workers+' עובדים</span>';
...
// ============================================================
var HELP_TEXT = {
  dash: 'עדכן את כמות היחידות שהושלמו בכל נתיב. לחץ "עדכן" על כל כרטיס.',
  stops: 'פתח עצירה כשהנתיב עצר. בחר נתיב וסיבה. סגור כשחוזרים לעבודה.',
  pack: 'בקש שיבוץ קרטונים למשטח. בחר נתיב, משטח, סוג קרטון וכמות.',
  mat: 'הזמן חומר מהמחסן. הזן מק"ט וכמות. הסטאפ יעדכן כשהחומר הגיע.',
  alerts: 'התראות מערכת — עצירות, פיגורים, הזמנות חומר שממתינות.',
  mgr: 'פאנל מנהל — ניתוחים, סיכומים, היסטוריה ודוחות.'
};

function showHelp(tab){
  var txt=HELP_TEXT[tab]||'';
  if(!txt) return;
  var el=document.getElementById('help-toast');
  if(!el){
    el=document.createElement('div');
    el.id='help-toast';
    el.style.cssText='position:fixed;bottom:80px;left:50%;transform:translateX(-50%);background:var(--dark,#1E293B);color:#fff;padding:10px 16px;border-radius:10px;font-size:13px;max-width:280px;text-align:center;z-index:999;box-shadow:0 4px 12px rgba(0,0,0,.3);transition:opacity .3s';
...
// ============================================================
// 2. VALIDATION - flexible, warns don't block
// ============================================================
var pendingOverride = null;

function validateUpdate(lineId, done, reason, isAdvance){
  var calc=calcLine(lineId);
  var target=calc.combined||0;
  if(!target||done<=0) return true; // no validation needed

  var pct=Math.round(done/target*100);

  // If flagged as advance work - save directly
  if(isAdvance){
    saveUpdate(lineId, done, reason, true);
    return false;
  }

  // Warn if >130% of target
  if(pct>130){
    pendingOverride={lineId:lineId,done:done,reason:reason};
    var line=LINES.filter(function(l){return l.id===lineId;})[0];
    document.getElementById('val-line-name').textContent=line?line.name:'';
    document.getElementById('val-amount').textContent=done+' יחידות ('+pct+'% מהיעד)';
    document.getElementById('val-modal').classList.add('open');
    return false; // pause, wait for user confirmation
  }
...
function confirmAdvance(){
  ...
  saveUpdate(pendingOverride.lineId, pendingOverride.done, pendingOverride.reason, true);
  pendingOverride=null;
  document.getElementById('val-modal').classList.remove('open');
}

function saveUpdate(lineId, done, reason, isAdvance){
  if(!reports[lineId]) reports[lineId]=[];
  reports[lineId].push({done:done, reason:reason, time:nowTime(), advance:isAdvance||false});
  saveState();syncToServer();runAlerts();renderDash();
  var now2=new Date();checkPendingUpdates(now2.getHours(),now2.getMinutes());
  showT(isAdvance?'עדכון נשמר (קידום עבודה)':'עדכון נשמר!');
}
...
function logAction(type, details){
  actionLog.unshift({type:type, details:details, time:nowTime(), ts:Date.now()});
  if(actionLog.length>200) actionLog=actionLog.slice(0,200);
  store.setItem('ops_log', JSON.stringify(actionLog));
}

function loadActionLog(){
  try{actionLog=JSON.parse(store.getItem('ops_log')||'[]');}catch(e){actionLog=[];}
}

function renderActionLog(){
  var el=document.getElementById('action-log');
  if(!el) return;
  if(!actionLog.length){el.innerHTML='<div class="empty">אין פעולות עדיין</div>';return;}

  var search=((document.getElementById('log-search')||{}).value||'').toLowerCase();
  var filtered=search?actionLog.filter(function(a){
    return (a.details||'').toLowerCase().includes(search)||(a.type||'').toLowerCase().includes(search);
  }):actionLog;

  var typeColors={update:'var(--blue)',stop:'var(--red)',material:'var(--yellow)',pack:'var(--green)',advance:'var(--purple,#7C3AED)'};
  var typeLabels={update:'עדכון',stop:'עצירה',material:'חומר',pack:'אריזה',advance:'קידום'};

  var html='<input type="text" id="log-search" placeholder="חפש פעולה..." oninput="renderActionLog()" style="width:100%;background:var(--surface);border:1px solid var(--border);border-radius:7px;padding:7px 10px;color:var(--text);font-size:12px;margin-bottom:8px">';

  filtered.slice(0,50).forEach(function(a){
    var col=typeColors[a.type]||'var(--dim)';
    ...
    html+='<span style="font-size:9px;font-family:var(--mono);color:var(--faint);min-width:38px">'+a.time+'</span>';
    html+='<span style="font-size:10px;padding:1px 7px;border-radius:10px;background:'+col+';color:#fff;font-weight:700;min-width:36px;text-align:center">'+lbl+'</span>';
    html+='<span style="font-size:12px;color:var(--text)">'+a.details+'</span>';
    html+='</div>';
  });
