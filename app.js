...
function mergeReports(local, remote){
  ...
  LINES.forEach(function(l){
    ...
    var remLast=rem[rem.length-1];
    var locDone=locLast?locLast.done||0:0;
    var remDone=remLast?remLast.done||0:0;
...
function syncToServer(){
  ...
  sbFetch('ops_data?select=key,value&key=in.(stoppages,pallets,reports,order_rows)').then(function(rows){
    ...
    if(map.reports){try{reports=mergeReports(reports,JSON.parse(map.reports));}catch(e){}}
...
function syncFromServer(){
  ...
  sbFetch('ops_data?select=key,value').then(function(rows){
    ...
    if(map.reports){try{
      var remoteRep=JSON.parse(map.reports);
      // Always merge: keep whichever has more done per line
      LINES.forEach(function(l){
        var local=reports[l.id]||[];
        var remote=remoteRep[l.id]||[];
        var localLast=local[local.length-1];
        var remoteLast=remote[remote.length-1];
        var localDone=localLast?localLast.done||0:0;
        var remoteDone=remoteLast?remoteLast.done||0:0;
        if(remoteDone>localDone) reports[l.id]=remote;
        else if(localDone===0&&remote.length>0) reports[l.id]=remote;
      });
    }catch(e){}}
...
setInterval(function(){
  sbFetch('ops_data').then(function(data){
    ...
    if(map.reports) try{
      var remoteRepBG=JSON.parse(map.reports);
      LINES.forEach(function(l){
        var local=reports[l.id]||[];
        var remote=remoteRepBG[l.id]||[];
        var localDone=(local[local.length-1]||{}).done||0;
        var remoteDone=(remote[remote.length-1]||{}).done||0;
        if(remoteDone>localDone) reports[l.id]=remote;
      });
    }catch(e){}
    if(map.order_rows) try{
