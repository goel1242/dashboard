...
function syncFromServer(){
  ...
  sbFetch('ops_data?select=key,value').then(function(rows){
    ...
    if(map.order_rows) orderRows=JSON.parse(map.order_rows);
...
function renderDash(){
  ...
  list.innerHTML=calcs.map(function(c){
    ...
    return '<div class="lc s'+c.statusIdx+'" onclick="openLineDetail(\''+c.id+'\')" style="cursor:pointer">'+
...
function parseWorkPlan(rows){
  ...
  rows.forEach(function(row){
    ...
    // Order tracking rows
    if(startDate.getTime()===today.getTime()&&qty>0){
      var isRed=endDate<today;
    ...
    orderRows[lineId].push({sku:sku,desc:desc,qty:qty,done:0,status:'open',isRed:isRed,note:'',
      order:orderNum,orderLine:orderLine,customer:customer,expiry:expiry,skuB:skuB});
    }
  });
  if(matched===0){if(status){status.textContent='אין הזמנות להיום';status.style.color='var(--yellow)';}return;}
  ...
  if(status){status.textContent=matched+' הזמנות נמצאו';status.style.color='var(--green)';}
  saveOrderRows();
}
...
function renderLineRows(){
  ...
  if(!rows.length){
    el.innerHTML = '<div class="empty">טען תוכנית עבודה בטאב מנהל</div>';
  ...
  var doneCnt = rows.filter(function(r){return r.status==='done';}).length;
  var pct = totalQty>0?Math.round(totalDone/totalQty*100):0;
  var barCol = pct>=100?'var(--green)':pct>=60?'var(--blue)':'var(--yellow)';
  var redCnt = rows.filter(function(r){return r.isRed;}).length;

  if(prog) prog.textContent = totalDone+'/'+totalQty+' יח ('+pct+'%)';
  ...
  html += '<div style="height:100%;background:'+barCol+';width:'+Math.min(pct,100)+'%;transition:width .3s"></div></div>';

  // Stats + search
  html += '<div style="display:flex;gap:8px;flex-wrap:wrap;margin-bottom:8px;align-items:center">';
  html += '<span style="font-size:11px;padding:3px 9px;border-radius:12px;background:var(--green-bg);color:var(--green);border:1px solid rgba(5,150,105,.2)">'+doneCnt+' הושלמו</span>';
  html += '<span style="font-size:11px;padding:3px 9px;border-radius:12px;background:var(--surface);color:var(--dim);border:1px solid var(--border)">'+(rows.length-doneCnt)+' נותרו</span>';
  if(redCnt>0) html += '<span style="font-size:11px;padding:3px 9px;border-radius:12px;background:var(--red-bg);color:var(--red);border:1px solid rgba(220,38,38,.2)">'+redCnt+' אדומות</span>';
  html += '</div>';

  // Search box
  html += '<input type="text" id="lt-search" placeholder="חפש מק-ט / הזמנה / לקוח..." oninput="renderLineRows()" value="'+search+'" style="width:100%;background:var(--surface);border:1px solid var(--border);border-radius:8px;padding:8px 12px;color:var(--text);font-size:13px;margin-bottom:10px">';

  // Hourly log
  html += '<div id="hourly-log-'+currentLine+'"></div>';
  ...
  filtered.forEach(function(r){
    ...
    var isRed = r.isRed;
    var rowBg = isDone?'rgba(5,150,105,.04)':isPartial?'rgba(217,119,6,.04)':isRed?'rgba(220,38,38,.06)':'var(--card)';
    var borderCol = isRed?'rgba(220,38,38,.25)':'var(--border)';

    html += '<div style="background:'+rowBg+';border:1px solid '+borderCol+';border-radius:10px;padding:12px 14px;margin-bottom:8px">';

    // Row 1: order + line + customer
    html += '<div style="margin-bottom:4px">';
    if(r.order) html += '<span style="font-size:14px;font-weight:700;font-family:var(--mono);color:'+(isRed?'var(--red)':'var(--dim)')+'">'+r.order+(r.orderLine?' / '+r.orderLine:'')+'</span>';
    if(r.customer) html += ' <span style="font-size:15px;font-weight:800;color:'+(isRed?'var(--red)':'var(--text)')+';margin-right:8px"> '+r.customer+'</span>';
    html += '</div>';

    // Row 2: SKU + desc
    html += '<div style="margin-bottom:4px">';
    html += '<span style="font-size:17px;font-weight:900;font-family:var(--mono);color:'+(isRed?'var(--red)':'var(--text)')+'">'+( r.sku||'-')+'</span>';
    if(r.desc) html += ' <span style="font-size:14px;color:var(--dim)">'+r.desc+'</span>';
    html += '</div>';

    // Row 3: skuB + expiry
    var extras = [];
    if(showSkuB&&r.skuB) extras.push('<span style="font-size:12px;padding:2px 8px;background:var(--blue-bg);color:var(--blue);border-radius:5px">בן: '+r.skuB+'</span>');
    if(showExpiry&&r.expiry) extras.push('<span style="font-size:12px;padding:2px 8px;background:'+(isRed?'var(--red-bg)':'var(--surface)')+';color:'+(isRed?'var(--red)':'var(--dim)')+';border-radius:5px">פק"ע: '+r.expiry+'</span>');
    if(extras.length) html += '<div style="display:flex;gap:6px;flex-wrap:wrap;margin-bottom:8px">'+extras.join('')+'</div>';

    if(r.note) html += '<div style="font-size:12px;color:var(--yellow);margin-bottom:6px">'+r.note+'</div>';

    // Action row
    html += '<div style="display:flex;justify-content:space-between;align-items:center">';
    // Qty display
    if(isDone||isPartial){
      html += '<span style="font-size:20px;font-weight:900;font-family:var(--mono);color:'+(isDone?'var(--green)':'var(--yellow)')+'">'+r.done+'/'+r.qty+'</span>';
    } else {
      html += '<span style="font-size:17px;font-weight:700;font-family:var(--mono);color:var(--dim)">'+r.qty+' יח</span>';
    }
    // Buttons - right side
    html += '<div style="display:flex;gap:8px;align-items:center">';
    if(isDone||isPartial){
      html += '<button data-lid="'+currentLine+'" data-idx="'+realIdx+'" onclick="undoRowMark(this.dataset.lid,parseInt(this.dataset.idx));renderLineRows()" style="font-size:11px;padding:4px 10px;background:var(--surface);border:1px solid var(--border);border-radius:6px;color:var(--faint);cursor:pointer">בטל</button>';
    } else {
      html += '<button data-lid="'+currentLine+'" data-idx="'+realIdx+'" onclick="toggleLtPartial(this)" style="width:48px;height:44px;background:var(--yellow-bg);border:1px solid rgba(217,119,6,.3);border-radius:8px;color:var(--yellow);font-size:20px;font-weight:900;cursor:pointer">~</button>';
      html += '<button data-lid="'+currentLine+'" data-idx="'+realIdx+'" onclick="markRowDone(this.dataset.lid,parseInt(this.dataset.idx));renderLineRows()" style="width:48px;height:44px;background:var(--green);border:none;border-radius:8px;color:#fff;font-size:22px;font-weight:900;cursor:pointer">&#10003;</button>';
    }
    html += '</div></div>';

    // Partial input
    if(!isDone){
      html += '<div style="display:none;align-items:center;gap:8px;margin-top:8px">';
      html += '<input type="number" id="lt-partial-'+realIdx+'" inputmode="numeric" placeholder="כמה?" min="1" max="'+r.qty+'" style="flex:1;background:var(--surface);border:1px solid var(--border);border-radius:8px;padding:8px 12px;color:var(--text);font-size:18px;font-family:var(--mono);text-align:center">';
      html += '<button data-lid="'+currentLine+'" data-idx="'+realIdx+'" onclick="markLtPartial(this.dataset.lid,parseInt(this.dataset.idx))" style="padding:8px 16px;background:var(--yellow);border:none;border-radius:8px;color:#fff;font-size:14px;font-weight:700;cursor:pointer">שמור</button>';
      html += '</div>';
    } else {
