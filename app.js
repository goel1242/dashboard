...
function syncFromServer(){
  ...
  sbFetch('ops_data?select=key,value').then(function(rows){
    ...
    if(map.order_rows){try{
      var sr2=JSON.parse(map.order_rows);
      if(Object.keys(sr2).length>0){
        var localTS2=window._orderRowsTS||parseInt(store.getItem('ops_order_rows_ts')||'0');
        var remoteTS2=map.order_rows_ts?parseInt(map.order_rows_ts):0;
        if(remoteTS2>localTS2+1000||localTS2===0){
...
setInterval(function(){
  sbFetch('ops_data').then(function(data){
    ...
    if(map.order_rows) try{
      var sr=JSON.parse(map.order_rows);
      if(Object.keys(sr).length>0){
        var localTS=window._orderRowsTS||parseInt(store.getItem('ops_order_rows_ts')||'0');
        var remoteTS=map.order_rows_ts?parseInt(map.order_rows_ts):0;
        // Only overwrite if remote is newer than local (with 5 second grace)
        if(remoteTS>localTS+5000||localTS===0){
