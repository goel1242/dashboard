...
function renderDash(){
  ...
  list.innerHTML=calcs.map(function(c){
    ...
    return '<div class="lc s'+c.statusIdx+'">'+
      '<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:10px">'+
        '<div style="display:flex;align-items:center;gap:7px">'+
          ...
          reasonTag+
        '</div>'+
        '<button data-lid="'+c.id+'" onclick="quickUpdate(this.dataset.lid)" style="font-size:11px;padding:4px 12px;background:var(--blue-bg);border:1px solid rgba(37,99,235,.3);border-radius:7px;color:var(--blue);cursor:pointer;font-weight:600">עדכן</button>'+
      '</div>'+
      '<div style="display:grid;grid-template-columns:repeat(3,1fr);gap:6px;margin-bottom:10px">'+
        '<div class="ni"><div class="ni-v" style="color:var(--blue)">'+(c.combined||'-')+'</div><div class="ni-l">'+(c.rrU>0?'יעד+אדום':'יעד יומי')+'</div></div>'+
        '<div class="ni"><div class="ni-v" style="color:var(--faint)">'+(c.combCum||'-')+'</div><div class="ni-l">תכנון כעת</div></div>'+
        '<div class="ni"><div class="ni-v" style="color:var(--green)">'+(c.done!==null?c.done:'-')+'</div><div class="ni-l">בוצע</div></div>'+
      '</div>'+
      ...
      '</div>'+
      '<div style="font-size:10px;color:var(--faint);font-family:var(--mono);margin-top:18px">עדכון: '+(c.last?c.last.time:'טרם דווח')+'</div>'+
      rrHtml+
    '</div>';
  }).join('');
}
