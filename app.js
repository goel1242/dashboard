...
function renderSetupRequests(){
  ...
  var open=materialRequests.filter(function(r){
    if(r.status!=='pending'&&r.status!=='inProgress') return false;
    // Safety net: even if the automatic day-reset was somehow missed, don't show requests
    // left open from a previous day as if they were today's - stale data, not "still waiting"
    var reqDay=new Date(r.createdTs||Date.now()).toDateString();
    return reqDay===_todaySetup;
  });
  ...
  var handled=materialRequests.filter(function(r){return r.status==='arrived'||r.status==='partial'||r.status==='nostock';})
